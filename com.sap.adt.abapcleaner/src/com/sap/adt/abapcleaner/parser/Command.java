...
public class Command {
	...
	public final boolean isRapHandlerMethodDef() {
		Token firstCode = getFirstCodeToken();
		if (firstCode == null)
			return false;
		Token forToken = firstCode.getLastTokenOnSiblings(true, "METHODS", TokenSearch.ANY_IDENTIFIER, TokenSearch.makeOptional("FINAL"), "FOR");
		if (forToken == null)
			return false;
		
		if (forToken.matchesOnSiblings(true, "FOR", "EVENT|TESTING")) {
			// event handlers and test methods:
			// - FOR EVENT OF
			// - FOR EVENT ... OF ...
			// - FOR TESTING ...
			return false;

		} else if (forToken.matchesOnSiblings(true, "FOR", "ENTITY|DETERMINE|GLOBAL|INSTANCE|AUTHORIZATION|FEATURES|LOCK|MODIFY|NUMBERING|PRECHECK|READ|VALIDATE")) {
			// RAP handlers:
			// - FOR ENTITY EVENT
