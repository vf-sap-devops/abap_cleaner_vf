...
/**
 * <p>
 * From ABAP cleaner perspective, a Token is the smallest relevant unit of ABAP
 * code: A {@link Command} consists of either a single Token or a sequence of
 * Tokens. {@link #toString()} exactly restores the ABAP source code from which
 * a Token was parsed.
 * </p>
 * 
 * <p>
 * Each Token stores the number of {@link #lineBreaks} and {@link #spacesLeft}
 * that separates it from the previous Token, allowing ABAP cleaner to easily
 * modify whitespace. In cleaning, a Token may be moved around, but its
 * {@link #text} is usually not changed.
 * </p>
 * 
 * <p>
 * The attributes {@link #prev} and {@link #next} provide the sequence of Tokens
 * in order of their appearance within the code; at the same time, a
 * hierarchical view of the Tokens inside a {@link Command} is provided via the
 * Token attributes {@link #parent}, {@link #firstChild}, {@link #lastChild},
 * {@link #prevSibling} and {@link #nextSibling}: Any Tokens inside parentheses
 * ( ... ) and brackets [ ... ] - or inside braces { ... } in string templates -
 * are considered 'child' Tokens; in such cases, the Token that ends with '(',
 * '[' or '{' is the {@link #parent}, while the '}', ')' or ']' Token is the
 * parent's {@link #nextSibling}. However, parentheses with no spaces and only
 * an integer literal inside them - such as 'char(8)' - are considered a single
 * Token (with no parent/child relationship).
 * </p>
 * 
 * <p>
 * More detailed description of what a Token is:
 * </p>
 * <ul>
 * <li>a Token is everything separated with whitespace (or until . , ' ` "); its
 * text includes final ( [, but not . or , or :</li>
 * <li>expressions linked with - ~ -&gt; =&gt; are one Token</li>
 * <li>string literals etc. such as 'text' or |text{ or }text| or `text` are one
 * Token each</li>
 * <li>a comment line (or the " comment at the end of a code line) is one single
 * Token, including the comment sign</li>
 * <li>) and ] are one Token respectively</li>
 * <li>= ?= += -= *= /= is one Token each</li>
 * <li>a pragma such as ##NO_TEXT is one Token</li>
 * <li>the final . or , as well as the chain colon : is a separate Token, even
 * if it is attached to the previous Token</li>
 * </ul>
 */
public class Token {
	...
	final void setNext(Token value) {
		next = value;
	}

	public final Token getPrevSibling() {
		return prevSibling;
	}
	...
	public final Token getFirstChild() {
		return firstChild;
	}

	final void setFirstChild(Token value) {
		firstChild = value;
	}

	public final Token getLastChild() {
		return lastChild;
	}

	final void setLastChild(Token value) {
		lastChild = value;
	}

	public final Token getFirstCodeChild() {
		return (firstChild == null || firstChild.isCode()) ? firstChild : firstChild.getNextCodeSibling();
	}

	public final Token getLastCodeChild() {
		return (lastChild == null || lastChild.isCode()) ? lastChild : lastChild.getPrevCodeSibling();
	}
	...
	public final boolean closesLevel() {
		return closesLevel;
	}

	final void setClosesLevel(boolean value) {
		closesLevel = value;
	}

	public final boolean getOpensLevel() {
		return opensLevel;
	}

	final void setOpensLevel(boolean value) {
		opensLevel = value;
	}
	...
	public final boolean isQuotMarkComment() {
		return isComment() && AbapCult.stringStartsWith(text, ABAP.COMMENT_SIGN_STRING);
	}

	public final boolean isCommentLine() {
		// in DDL, a line may contain multiple comments, e.g. /* comment */ // comment
		// however, unlike Command.isCommentLine(), Token.isCommentLine() only returns
		// true if this is the only Token on this line
		return isComment() && isFirstTokenInLine() && (next == null || next.lineBreaks > 0);
	}

	public final boolean isQuotMarkCommentLine() {
		return isCommentLine() && isQuotMarkComment();
	}

	public final boolean isDdlLineEndComment() {
		return isComment() && textStartsWithAny(DDL.LINE_END_COMMENT, DDL.LINE_END_MINUS_COMMENT);
	}

	public final boolean isAbapDocComment() {
		// ABAP Doc comments are expected to be comment lines, however, it is
	...
	public final boolean isAbapDocCommentLine() {
		return isCommentLine() && AbapCult.stringStartsWith(text, ABAP.ABAP_DOC_SIGN);
	}

	public final boolean isAsteriskCommentLine() {
		return isCommentLine() && AbapCult.stringStartsWith(text, ABAP.LINE_COMMENT_SIGN_STRING);
	}

	public final boolean isCommentAfterCode() {
		return isComment() && (prev != null) && (lineBreaks == 0);
	}

	public final boolean isCommentAfterPeriod() {
		return isComment() && (prev != null) && (lineBreaks == 0) && (prev.isPeriod());
	}

	public final boolean isPseudoComment() {
		// returns true if the comment starts with "#EC (this is NOT case-sensitive,
		// i.e. Extended Check also accepts "#ec and "#Ec;
		// however, the following word must be upper case: Extended Check accepts "#ec
		// NEEDED but it does NOT accept "#EC needed )
		return isComment() && AbapCult.stringStartsWith(text, ABAP.PSEUDO_COMMENT_EC_PREFIX);
	}

	public final boolean isPseudoCommentAfterCode() {
		return isPseudoComment() && (prev != null) && (lineBreaks == 0);
	}
	...
	public final boolean isIntegerLiteral() {
		return isLiteral() && !isStringLiteral();
	}

	public final boolean isFloatLiteral() {
		return isLiteral() && !StringUtil.isNullOrEmpty(text) && text.charAt(0) == ABAP.QUOT_MARK
				&& text.charAt(text.length() - 1) == ABAP.QUOT_MARK
	...
	public final boolean isOtherOp() {
		return (type == TokenType.OTHER_OP);
	}

	public final boolean isClosingParenthesisOrBracket() {
		return (type == TokenType.OTHER_OP && (text.equals(")") || text.equals("]")));
	}
	...
	public final boolean isKeyword(String text) {
		return isKeyword() && textEquals(text);
	}

	public final boolean isAnyKeyword(String... compareTexts) {
		return isKeyword() && textEqualsAny(compareTexts);
	}

	public final boolean isTextualComparisonOp() {
		return (type == TokenType.COMPARISON_OP) && !StringUtil.isNullOrEmpty(text)
				&& Character.isLetter(text.charAt(0));
	}

	public final boolean isPragma() {
		return (type == TokenType.PRAGMA);
	}

	public final boolean isPragmaOrComment() {
		return isPragma() || isComment();
	}

	/** returns true if the Token is NOT a comment or pragma */
	public final boolean isCode() {
		return (type != TokenType.COMMENT) && (type != TokenType.PRAGMA);
	}
	...
	public final boolean isCommaOrPeriod() {
		return isComma() || isPeriod();
	}

	public final boolean isChainColon() {
		return (type == TokenType.COLON);
	}

	public final boolean isTextSymbol() {
		return (type == TokenType.KEYWORD && ABAP.mayBeTextSymbol(text));
	}

	final boolean getMayBeIdentifier() {
		return (type == TokenType.IDENTIFIER) || isKeyword() || isTextualComparisonOp();
	}

	public final boolean isTextFieldLiteral() {
		// also ensure that the literal ends with ' to exclude literals with text
		// element IDs: 'text'(idf)
		return isLiteral() && !StringUtil.isNullOrEmpty(text) && text.length() >= 2 && text.charAt(0) == ABAP.QUOT_MARK
				&& text.charAt(text.length() - 1) == ABAP.QUOT_MARK;
	}
	...
	public final boolean isCharacterLiteral() {
		return isLiteral() && !StringUtil.isNullOrEmpty(text)
				&& (text.charAt(0) == ABAP.QUOT_MARK || text.charAt(0) == ABAP.QUOT_MARK2);
	}

	public final boolean isStringLiteral() {
		return isLiteral() && !StringUtil.isNullOrEmpty(text)
				&& (text.charAt(0) == ABAP.QUOT_MARK || text.charAt(0) == ABAP.QUOT_MARK2 || text.charAt(0) == ABAP.PIPE
						|| text.charAt(0) == ABAP.BRACE_CLOSE);
	}
	...
	public final boolean isDdlStringLiteral() {
		return isLiteral() && !StringUtil.isNullOrEmpty(text) && text.charAt(0) == DDL.QUOT_MARK
				&& text.charAt(text.length() - 1) == DDL.QUOT_MARK;
	}

	final boolean startsStringTemplate() {
		return isLiteral() && !StringUtil.isNullOrEmpty(text) && text.charAt(0) == ABAP.PIPE;
	}

	final boolean endsStringTemplate() {
		return isLiteral() && !StringUtil.isNullOrEmpty(text) && text.charAt(text.length() - 1) == ABAP.PIPE;
	}

	final boolean startsWithLetter() {
		return !StringUtil.isNullOrEmpty(text) && Character.isLetter(text.charAt(0));
	}

	public final boolean startsEmbeddedExpression() {
		return isStringLiteral() && text.charAt(text.length() - 1) == ABAP.BRACE_OPEN;
	}

	public final boolean endsEmbeddedExpression() {
		return isStringLiteral() && text.charAt(0) == ABAP.BRACE_CLOSE;
	}

	public final boolean isAssignmentOperator() {
		return (type == TokenType.ASSIGNMENT_OP);
	}

	public final boolean isComparisonOperator() {
		return (type == TokenType.COMPARISON_OP);
	}

	public final boolean isComparisonOperator(String text) {
		return isComparisonOperator() && textEquals(text);
	}

	public final boolean isAnyComparisonOperator(String... compareTexts) {
		return isComparisonOperator() && textEqualsAny(compareTexts);
	}
	...
	public final boolean hasCodeChildren() {
		return (firstChild != null && (firstChild.isCode() || firstChild.getNextCodeSibling() != null));
	}

	public final boolean isPrecededByWhitespace() {
		return (lineBreaks > 0 || spacesLeft > 0);
	}

	public final boolean isFirstTokenInCommand() {
		return (parentCommand != null && parentCommand.firstToken == this);
	}

	public final boolean isFirstTokenInCode() {
		return isFirstTokenInCommand() && parentCommand.isFirstCommandInCode();
	}

	public final boolean isLastTokenInCommand() {
		return (parentCommand != null && parentCommand.lastToken == this);
	}

	public final boolean isFirstTokenInLine() {
		return lineBreaks > 0 || (sourceLineNum == 1 && isFirstTokenInCommand());
	}

	public final boolean isLastTokenInLineExceptComment() {
		return next == null || (next.lineBreaks > 0 || next.isCommentAfterCode());
	}

	public final boolean isOnlyTokenInLine() {
		return isFirstTokenInLine() && (next == null || next.lineBreaks > 0);
	}

	public final boolean isOnlyTokenInCommand() {
		return isFirstTokenInCommand() && isLastTokenInCommand();
	}

	public final boolean opensInlineDeclaration() {
		return isAnyKeyword("DATA(", "FINAL(", "FIELD-SYMBOL(", "@DATA(", "@FINAL(") && next != null
				&& next.isAttached();
	} // by contrast, 'REF data( lv_company_code )' is NOT an inline declaration,
		// because the next Token is NOT attached!

	public final boolean opensTableExpression() {
		return opensLevel && isIdentifier() && textEndsWith("[");
	}

	public final boolean opensInlineDeclarationForFieldSymbol() {
		return isAnyKeyword("FIELD-SYMBOL(") && next != null && next.isAttached();
	};
	...
	public final String getTypeAndTextForErrorMessage() {
		return type.toString() + " '" + text + "'";
	}

	public final String getSourceName() {
		return (parentCommand == null) ? null : parentCommand.getSourceName();
	}

	public final boolean isDdlAnnotation() {
		return text.length() > 0 && text.charAt(0) == DDL.ANNOTATION_SIGN;
	}

	/**
	 * Returns true if the Token text equals the supplied comparison text. To check
	 * the Token type at the same time, use {@link #isKeyword(String)} or
	 * {@link #isComparisonOperator(String)} (see list of tokens classified as
	 * comparison operators in {@link ABAP#isComparisonOperator(String, boolean)})
	 */
	...
	/**
	 * <p>
	 * Creates a new ABAP Token with the supplied {@link TokenType}. This overload
	 * should be preferred, especially in cases where the {@link TokenType} is
	 * known, but ambiguous:
	 * </p>
	 * <ul>
	 * <li>{@link TokenType#IDENTIFIER} if the identifier might look like an ABAP
	 * keyword;</li>
	 * <li>{@link TokenType#ASSIGNMENT_OP} and {@link TokenType#COMPARISON_OP} for
	 * the equals sign '='.</li>
	 * </ul>
	 * <p>
	 * Rules that use this method to create an ABAP keyword or an identifier should
	 * override {@link Rule#getDependentRules()} and return
	 * {@link RuleID#UPPER_AND_LOWER_CASE}.
	 * </p>
	 */
	public static Token createForAbap(int lineBreaks, int spacesLeft, String text, TokenType type, int sourceLineNum) {
		...
		if (token.type != type) {
			if ((token.type == TokenType.KEYWORD && type == TokenType.IDENTIFIER)
					...
					|| (token.type == TokenType.OTHER_OP && type == TokenType.IDENTIFIER
							&& ABAP.isPlaceholderInMacroDefinition(token.text))) {
				token.type = type;
				...
				// this case should NOT be ambiguous; possibly, a wrong TokenType was supplied
				// by the rule
				throw new IllegalArgumentException("Supplied TokenType " + type.toString()
						+ " unexpected for Token text '" + text + "'; using inferred TokenType " + token.type + "!");
			}
		}
		return token;
	}
	...
	/**
	 * Create a new ABAP Token. With this overload, the {@link TokenType} is
	 * automatically inferred from the text. Rules that use this method to create an
	 * ABAP keyword or an identifier should override
	 * {@link Rule#getDependentRules()} and return
	 * {@link RuleID#UPPER_AND_LOWER_CASE}.
	 */
	...
	private Token(int lineBreaks, int spacesLeft, String text, int sourceLineNum, Language language) {
		...
		// preliminarily determine the type; Command.finishBuild() may correct this
		// choice depending on the context of this Token
		// (at this point, we only need to identify comments and tell whether the Token
	...
	private static TokenType inferTypeFromAbapToken(String text) {
		if (AbapCult.stringStartsWith(text, ABAP.COMMENT_SIGN_STRING)) {
		...
		} else if (text.length() > 0 && (text.charAt(0) == ABAP.QUOT_MARK || text.charAt(0) == ABAP.QUOT_MARK2
				|| text.charAt(0) == ABAP.PIPE || text.charAt(0) == ABAP.BRACE_CLOSE)) {
			...
			// "=" will later be differentiated between assignment and comparison operator
			// in Command.finishBuild() / .distinguishOperators()
			...
			// some identifiers may be preliminarily categorized as keywords here, see
			// Command.finishBuild()
			...
			// text symbols such as 'TEXT-001' and 'TEXT-a01' are stored as one Token of
			// type KEYWORD and get special handling where needed
			return TokenType.KEYWORD;

		} else {
			// Assume .IDENTIFIER if at least one letter a-z A-Z or an underscore _ is
			// found, otherwise assume .OTHER_OP.
			// This first assumption may be corrected in TokenTypeRefinerRnd.refine(Command,
			// Token, Token), esp. if the
			// RND Parser categorizes the Token as .CAT_IDENTIFIER: in non-object-oriented
			// contexts, even the single
			// characters $ % ?, as well as sequences like &%$?*#> and <%$?*# could be valid
			// variable names,
			// cp. ABAP.isCharAllowedForVariableNames() and
			// ABAP.isCharAllowedForTypeNames()! However, all the above
			// categorizations as .COMMENT, .PRAGMA, .LITERAL etc. (except .KEYWORD) are
			// always correct, even in
	...
	private static TokenType inferTypeFromSqlScriptToken(String text) {
		// in non-ABAP sections (e.g. EXEC SQL ... ENDEXEC), everything except comments
		// gets TokenType.NON_ABAP (including literals)
	...
	public final void addNext(Token newToken) throws UnexpectedSyntaxException {
		...
		// the following check is deactivated, because a colon inside of parentheses or
		// brackets can be accepted,
		// as long as parentheses are NOT closed twice (which is checked below with 'if
		// (parent == null)')
		// if (parent != null && newToken.isChainColon())
		// throw new UnexpectedSyntaxException(this, "Unsupported syntax: Chain colon
		// ':' inside parentheses ( ... ) or brackets [ ... ] cannot be processed by " +
		// Program.PRODUCT_NAME + ". Please refactor this command first.");
		...
		if (opensLevel && !newToken.closesLevel) {
			// correct the token type of '*' in 'lv_any+4(*) = ...', which was initially
			...
			if (parent == null) {
				...
				if (parentCommand.containsChainColonInsideParentheses()) {
					msg = "Chain colons inside parentheses or brackets are currently not supported by "
							+ Program.PRODUCT_NAME + ". ";
					msg += "Please rewrite this ABAP statement manually before running " + Program.PRODUCT_NAME
							+ " on this code.";
				} else {
					msg = "The Token '" + newToken.text + "' cannot be added to '" + text
							+ "', because the latter has no parent Token.";
				}
	...
	/**
	 * Checks whether this Token and the following siblings match the supplied
	 * sequence of texts. Supplied texts may contain alternatives (separated with
	 * the TokenSearch.TEXT_MATCH_VARIANT_SEPARATOR |, e.g. "a|b|c"), use the
	 * {@link TokenSearch} constants ANY_LITERAL, ANY_IDENTIFIER etc. or its static
	 * method {@link TokenSearch#makeOptional(String)}.
	 */
	...
	/**
	 * Checks whether this Token and the following (child, sibling, or parent)
	 * Tokens match the supplied sequence of texts. Supplied texts may contain
	 * alternatives (separated with the TokenSearch.TEXT_MATCH_VARIANT_SEPARATOR |,
	 * e.g. "a|b|c"), use the {@link TokenSearch} constants ANY_LITERAL,
	 * ANY_IDENTIFIER etc. or its static method
	 * {@link TokenSearch#makeOptional(String)}.
	 */
	...
	public final Token getLastTokenOfSequence(boolean siblingsOnly, boolean skipCommentsAndPragmas, Token endToken,
			String... texts) {
		final char space = ' ';
		...
		while (textIndex < texts.length) {
			...
			boolean match;
			if (TokenSearch.ASTERISK.equals(text) || TokenSearch.ANY_NUMBER_OF_COMMENTS.equals(text)
					|| TokenSearch.MAX_ONE_NON_MATCHING_TOKEN.equals(text)) {
				skipMode = true;
				...
				// TODO: "|| ..." is required because the differentiation between assignment and
				// comparison operator in Command.finishBuild() / .distinguishOperators() is not
				// yet perfect:
				...
				for (String variant : variants) {
					// text may still contain spaces for several Tokens, e.g. "TRANSPORTING NO
					// FIELDS"
					Token testToken = token.getLastTokenOfPlainSequence(siblingsOnly, skipCommentsAndPragmas,
							StringUtil.split(variant, space, false));
					if (testToken != null) {
						match = true;
						token = testToken;
						break;
				...
				// FIELDS"
				Token testToken = token.getLastTokenOfPlainSequence(siblingsOnly, skipCommentsAndPragmas,
						StringUtil.split(text, space, false));
			...
			if (match) {
				...
				if (optionalMode) {
					// the optional text bit was not found; continue with the next text bit WITHOUT
					// proceeding to the next Token
					optionalMode = false;
					++textIndex;
					...
					// If we are searching for texts = { "END", "OF" }, and the code reads "end TYPE
					// ... END OF", then a mismatch
					// will be detected between texts[textIndex] = "OF" and Token.text = "TYPE". In
					// such a case, resume the matching
					// from the 'resumeSkipIndex', which points to texts[resumeSkipIndex] = "END",
					// but do NOT advance the token.
					// (Endless loops are prevented by the 'resumeSkipIndex < textIndex' condition.)
					textIndex = resumeSkipIndex;
					// a 'resumeSkipIndex' is only used if
					// TokenSearch.ANY_NUMBER_OF_NON_MATCHING_TOKENS is encountered:
					skipMode = true;
	...
	public final void removeFromCommand(boolean mayMoveFollowingLinesLeft, boolean skipIntegrityTest)
			throws UnexpectedSyntaxAfterChanges {
		if (hasChildren())
			throw new UnexpectedSyntaxAfterChanges(null, this,
					"Removing the Token '" + text + "' with its child Tokens is not yet supported!");
		...
		// before removing the Token, get some relationships and information for the
		// .addIndent() call below
		// if (isFirstTokenInLine)
		// mayMoveFollowingLinesLeft = false;
		Command command = parentCommand;
		Token nextToken = next;
		int length = spacesLeft + getTextLength();
		int rightPos = mayMoveFollowingLinesLeft ? getEndIndexInLine() : 0; // save performance if variable is not
																			// needed

		// prevent the next Token in the line from being moved up to the previous line,
		// esp. if the previous line ends
		// with a comment; if the to-be-removed Token is afterwards moved to a different
		// place, its new lineBreaks / spacesLeft
		...
		if (lineBreaks > 0 && next != null && next.lineBreaks == 0) {
			...
			if (mayMoveFollowingLinesLeft) {
				// for now, keep the next Token at its position, and adjust the length for the
				// .addIndent() call below
	...
	public final void insertParenthesesUpTo(Token tokenAfterParentheses, String openingParenthesisText,
			String closingParenthesisText) throws IntegrityBrokenException {
		...
		// if the tokenAfterParentheses is itself a closing parenthesis, insert a
		// temporary right sibling to the Token before it:
		// e.g., the Command is "CHECK a = 1 AND ( b = 1 OR b = 2 AND c = 3 ).", and
		// parentheses shall be added around "b = 2 AND c = 3";
		// then insert "T" as temporary Token: "CHECK a = 1 AND ( b = 1 OR b = 2 AND c =
		// 3 T )." and remove it later
		...
		// first insert opening parenthesis as if it was a normal sibling (skipping the
		// integrity test)
		Token openingParenthesis = Token.create(lineBreaks, spacesLeft, openingParenthesisText, sourceLineNum,
				parentCommand.getLanguage());
		...
		// int spacesLeft = tokenAfterParentheses.isPrecededByWhitespace() ?
		// tokenAfterParentheses.spacesLeft : 1;
		Token closingParenthesis = Token.create(0, 1, closingParenthesisText, tokenAfterParentheses.sourceLineNum,
				parentCommand.getLanguage());
	...
	public final void removeParentheses() throws IntegrityBrokenException, UnexpectedSyntaxException {
		...
		if (nextAfterParentheses != null && nextAfterParentheses.isCommaOrPeriod()
				&& !nextAfterParentheses.prev.isComment())
			nextAfterParentheses.setWhitespace(0, 0);

		parentCommand.testReferentialIntegrity(true);
	}

	public final void appendParenthesesUpTo(Token tokenAfterParentheses, boolean keepPragmasBehindParens)
			throws UnexpectedSyntaxAfterChanges {
		if (tokenAfterParentheses == null)
			throw new NullPointerException("tokenAfterParentheses");
		...
		if (hasChildren() || opensLevel || textEndsWith("(") || next == null)
			throw new UnexpectedSyntaxAfterChanges(null, this,
					"Insertion of parentheses not supported for the Token '" + text + "' because it has child Tokens!");

		// keep pragmas (and comments between those pragmas) at the end behind the
		// parentheses?
		...
		Token closingParenthesis = Token.create(tokenAfterParentheses.lineBreaks, spacesLeft, ")",
				tokenAfterParentheses.sourceLineNum, parentCommand.getLanguage());
	...
	/**
	 * inserts the supplied Token (including its possible children) as a sibling
	 * before this Token
	 * 
	 * @param newToken
	 ...
	 * @throws IntegrityBrokenException
	 */
	public final Token insertLeftSibling(Token newToken, boolean moveFollowingLinesRight)
			throws IntegrityBrokenException {
	 ...
	 * @throws IntegrityBrokenException
	 */
	public final Token insertLeftSibling(Token newToken, boolean moveFollowingLinesRight, boolean skipIntegrityTest)
			throws IntegrityBrokenException {
	 ...
	 * @throws IntegrityBrokenException
	 */
	public final Token insertLeftSibling(Token newToken, boolean moveFollowingLinesRight, Token addIndentEndToken,
			boolean skipIntegrityTest) throws IntegrityBrokenException {
		if (newToken == null)
			throw new NullPointerException("newToken");
		if (closesLevel)
			throw new IntegrityBrokenException(this, "cannot insert a left sibling to a token that closes a level");

		int oldStartIndex = moveFollowingLinesRight ? getStartIndexInLine() : 0; // save performance if information is
																					// not needed below
		...
		if (moveFollowingLinesRight)
			parentCommand.addIndent(newToken.spacesLeft + newToken.getTextLength(), oldStartIndex, this,
					addIndentEndToken, true);
	...
	/**
	 * inserts the supplied Token as the next sibling, or first child after this
	 * Token
	 * 
	 * @param newToken
	 * @throws IntegrityBrokenException
	...
	/**
	 * inserts the supplied Term as the next sibling, or first child after this
	 * Token
	 */
	 ...
	 * after this Token
	 */
	public final Token insertRightSibling(Token newToken, boolean moveFollowingLinesRight)
			throws IntegrityBrokenException {
	 ...
	 * after this Token
	 */
	public final Token insertRightSibling(Token newToken, boolean moveFollowingLinesRight, boolean skipIntegrityTest)
			throws IntegrityBrokenException {
		if (newToken == null)
			throw new NullPointerException("newToken");
		if (opensLevel)
		...
		int oldStartIndex = moveFollowingLinesRight ? getEndIndexInLine() + 1 : 0; // save performance if information is
																					// not needed below
		...
		if (isComment() && newToken.lineBreaks == 0) {
			if (next == null) {
				throw new IntegrityBrokenException(this,
						"cannot insert a right sibling after the final comment in a command");
			} else {
		...
		if (moveFollowingLinesRight)
			parentCommand.addIndent(newToken.spacesLeft + newToken.getTextLength(), oldStartIndex, newToken, null,
					true);
	...
	/**
	 * inserts the supplied Term (including its possible children) as a sibling
	 * after this Token
	 * 
	 * @param newTerm
	 * @throws IntegrityBrokenException
	 ...
	 * @throws IntegrityBrokenException
	 */
	public final void insertRightSibling(Term newTerm, boolean moveFollowingLinesRight)
			throws IntegrityBrokenException {
	 ...
	 * @throws IntegrityBrokenException
	 */
	public final void insertRightSibling(Term newTerm, boolean moveFollowingLinesRight, boolean skipIntegrityTest)
			throws IntegrityBrokenException {
		if (newTerm == null)
		...
		Token lastTokenInNewTerm = newTerm.lastToken.lastChild != null ? newTerm.lastToken.lastChild
				: newTerm.lastToken; // TODO: can newTerm.lastToken have child Tokens at all?
		...
		if (moveFollowingLinesRight)
			parentCommand.addIndent(newTerm.firstToken.spacesLeft + newTermWidth, oldStartIndex, newTerm.lastToken,
					null);
	 ...
	 * To check the Token type at the same time, use
	 * {@link #isAnyKeyword(String...)} or
	 * {@link #isAnyComparisonOperator(String...)} (see list of tokens classified as
	 * comparison operators in {@link ABAP#isComparisonOperator(String, boolean)})
	 */
	...
	final TextBit[] toTextBits(int startIndex, boolean isInOOContext) {
		...
		if (type != TokenType.KEYWORD && type != TokenType.IDENTIFIER && type != TokenType.LITERAL
			...
			if (type == TokenType.LITERAL) {
				...
				// session variables are functionally used like variables (and therefore should
				// have TokenType.IDENTIFIER),
		...
		// literals may be linked to a text symbol ID, having the form 'literal
		// text'(idf), where 'idf' is always 3 characters long
		...
		if (isAbap && isTextSymbol()) {
			// text symbol IDs may be numeric (TEXT-001) or contain letters or _ (TEXT-a01,
			// TEXT-a_2)
			int prefixLength = ABAP.TEXT_SYMBOL_PREFIX.length();
			ColorType idColorType = ABAP.consistsOfDigitsOnly(text.substring(prefixLength)) ? ColorType.NUMBER
		...
		for (int i = 0; i < text.length(); ++i) {
			...
			if (isDdlOrDcl) {
				...
				// in some cases, initial '/' does NOT start an identifier with a namespace,
				// e.g. "ULINE AT /.", "ULINE AT /10(20)." or "ULINE at /pos(20)."
				if (i == 0 && c == '/' && (text.length() <= 1 || text.indexOf('/', 1) < 0)) {
					isIdentifier = false;
				}
			}
			if (i > 0 && isIdentifier != lastWasIdentifier) {
				// if the text bit is not part of the keyword or the identifier, it is
				// considered a token operator (e.g. "->", "=>", "(" etc.)
				ColorType bitType = lastWasIdentifier ? colType : opColorType;
				if (isDdlOrDcl && AbapCult.stringEqualsAny(true, text.substring(writtenPos, i), DDL.PROJECTION_PREFIX,
						DDL.PARAMETER_PREFIX))
		...
		if (writtenPos < text.length()) {
			ColorType bitType = lastWasIdentifier ? colType : opColorType;
			// in some cases, a Token of type IDENTIFIER contains a number, e.g. "ULINE AT
			// /10."
	...
	public final int getStartIndexInLine() {
		...
		// does this Command continue the line of the previous Command? (parentCommand
		// may be null if this Token is being moved)
	...
	/**
	 * if the Token is an identifier of type "structure-component" or
	 * "&lt;field-symbol&gt;-component", the structure or field-symbol variable is
	 * returned
	 * 
	 * @return
	 */
	public final String getStructureVariable() {
		if (!isIdentifier())
			return null;
		int selectorPos = text.lastIndexOf(ABAP.COMPONENT_SELECTOR); // *last*IndexOf() is used for cases of complex
																		// statements that have components at their end
		if (selectorPos <= 0)
			return null;
		else
	...
	public final Token getNextWhileComment() {
		return isComment() ? getNextNonCommentToken() : this;
	}

	public final Token getPrevWhileComment() {
		return isComment() ? getPrevNonCommentToken() : this;
	}

	/**
	 * returns this Token unless it is a pragma or a comment; otherwise the next
	 * 'code' Token
	 */
	...
	/**
	 * returns the previous sibling Token that is not a comment (but may be a
	 * pragma)
	 */
	...
	private void check(boolean value) throws IntegrityBrokenException {
		if (!value) {
			if (parentCommand == null) {
				...
				throw new IntegrityBrokenException(this,
						"Failed referential integrity test on Token '" + text + "' in Command starting at source line "
								+ Cult.format(parentCommand.getSourceLineNumStart()) + "!");
			}
		}
	}
	...
	/**
	 * Changes the Token's {@link #text}. Rules that use this method to change an
	 * ABAP keyword or an identifier should override
	 * {@link Rule#getDependentRules()} and return
	 * {@link RuleID#UPPER_AND_LOWER_CASE}.
	 * 
	...
	public static Token findEndOfLogicalExpression(Token firstTokenOfLogExpr) throws UnexpectedSyntaxException {
		...
		// see
		// https://ldcier1.wdf.sap.corp:44300/sap/public/bc/abap/docu?object=abenlogexp
	...
	public static Token findEndOfRelationalExpression(Token firstTokenOfRelExpr) throws UnexpectedSyntaxException {
		...
		// Predicate Functions
		// - for Character-Like Arguments, see
		// https://ldcier1.wdf.sap.corp:44300/sap/public/bc/abap/docu?object=abenpredicate_functions_strgs
		...
		// - for Table-Like Arguments, see
		// https://ldcier1.wdf.sap.corp:44300/sap/public/bc/abap/docu?object=abenpredicate_functions_tabs
		if (token.textEqualsAny("line_exists("))
			return token.getNextSibling().getNextCodeSibling();

		// read predicative method call, operand of predicate expression, or arithmetic
		// expression
		Term term1 = Term.createArithmetic(token);
		token = term1.getNextCodeToken();

		if (token.isKeyword("IS")) {
			// Predicate Expressions, see
			// https://ldcier1.wdf.sap.corp:44300/sap/public/bc/abap/docu?object=abenpredicate_expressions
			token = token.getNextCodeSibling();
			...
			// Comparison Expressions (rel_exp), see
			// https://ldcier1.wdf.sap.corp:44300/sap/public/bc/abap/docu?object=abenlogexp_comp
			...
			// assume Predicative Method Call, see
			// https://ldcier1.wdf.sap.corp:44300/sap/public/bc/abap/docu?object=abenpredicative_method_calls
			// - nothing to do
		}
		return token;
	}
	...
	private MemoryAccessType determineMemoryAccessType() {
		// Checks whether the token is at a write (or read-write) position, depending on
		// the ABAP commands; restrictions:
		// - does not consider declaration positions
		// - only explicitly considers write / read-write, because read is the default
		// result.
		// - 'TODO' comments in this method show sections or commands that are not yet
		// considered
		...
		if (firstToken.isAnyKeyword(Command.declarationKeywords)) {
			if (prevToken.isAnyKeyword(Command.declarationKeywordsReservingMemory)
				...
				// except for TYPES, the declared identifier is in a write or assignment
				// position
				return ABAP.isFieldSymbol(text) ? MemoryAccessType.ASSIGN_TO_FS_OR_DREF : MemoryAccessType.WRITE;
			} else if (prevToken.isIdentifier() && prevToken.getOpensLevel() && prevPrevToken != null
					&& (prevPrevToken.isAnyKeyword(Command.declarationKeywords) || prevPrevToken.isComma()
							|| prevPrevToken.isChainColon())) {
				...
				// e.g. "lc_length" and "lc_text" in "DATA lv_text TYPE c LENGTH lc_length VALUE
				// lc_text."
				return MemoryAccessType.READ;
		...
		// assignments of type 'var = ...' or 'itab[ ... ] = ...' (without inline
		// declarations)
		if (command.isAssignment(false, true) && nextToken.isAssignmentOperator()) {
			// in case of equals sign chaining 'a = b = c = 1.', move to the first receiving
			// variable
		...
		if (firstToken.isKeyword("MOVE-CORRESPONDING") && prevToken.isKeyword("TO")) {
			...
			// MOVE-CORRESPONDING [EXACT] itab1 TO itab2 [EXPANDING NESTED TABLES] [KEEPING
			// TARGET LINES].
			return MemoryAccessType.WRITE;
			...
			return MemoryAccessType.WRITE;
		} else if (firstToken.isAnyKeyword("CLEAR", "CLEAR:", "FREE", "FREE:") && parent == null
				&& !prevToken.isKeyword("WITH")) {
		...
		if (firstToken.isKeyword("CONCATENATE") && prevToken.isKeyword("INTO")) {
			// CONCATENATE {dobj1 dobj2 ...}|{LINES OF itab} INTO result [IN
			// {CHARACTER|BYTE} MODE] [SEPARATED BY sep] [RESPECTING BLANKS].
			return MemoryAccessType.WRITE;
			...
			return MemoryAccessType.WRITE;
				...
				&& (prevToken.isKeyword("RESULTS") || (prevToken.isAnyKeyword("COUNT", "OFFSET", "LENGTH")
						&& prevPrevToken != null && prevPrevToken.isKeyword("MATCH")))) {
			// MATCH COUNT mcnt, MATCH OFFSET moff, MATCH LENGTH mlen, RESULTS
			// result_tab|result_wa
			// cp.
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abapfind_options.htm
			return MemoryAccessType.WRITE;
		} else if (firstToken.isKeyword("FIND")) {
			...
			if (firstToken.matchesOnSiblings(true, TokenSearch.ASTERISK, "INTO")) {
				// obsolete syntax: REPLACE substring WITH new INTO dobj [IN {BYTE|CHARACTER}
				// MODE] [LENGTH len].
				if (prevToken.isKeyword("INTO")) {
					return MemoryAccessType.WRITE;
				}
			} else if (nextToken.isKeyword("WITH") || prevToken.isKeyword("RESULTS")
					|| (prevToken.isAnyKeyword("COUNT", "OFFSET", "LENGTH") && prevPrevToken != null
							&& prevPrevToken.isKeyword("REPLACEMENT"))) {
				// REPLACE [{FIRST OCCURRENCE}|{ALL OCCURRENCES} OF] pattern IN [section_of]
				// dobj WITH new [IN {CHARACTER|BYTE} MODE] REPLACEMENT COUNT rcnt, REPLACEMENT
				// OFFSET roff, REPLACEMENT LENGTH rlen
				// cp.
				// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abapreplace_options.htm
				return MemoryAccessType.WRITE;
			}
			...
			// cp.
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abapsplit.htm
			Token test = prevToken;
			while (test != null && (!test.isKeyword() || test.opensInlineDeclaration()))
				test = test.getPrevCodeSibling();
		...
		if (firstToken.matchesOnSiblings(true, "CONVERT", "DATE") && prevToken.isAnyKeyword("UTCLONG")) {
			// CONVERT DATE dat TIME tim [FRACTIONAL SECONDS fs] [DAYLIGHT SAVING TIME dst]
			// TIME ZONE tz INTO UTCLONG time_stamp.
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "CONVERT", "DATE") && prevToken.isAnyKeyword("STAMP")) {
			// CONVERT DATE dat [TIME tim [DAYLIGHT SAVING TIME dst]] INTO TIME STAMP
			// time_stamp TIME ZONE tz.
			// note that TIME ZONE is NOT a write position, although it is behind the INTO
			// keyword
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "CONVERT", "UTCLONG")
				&& prevToken.isAnyKeyword("DATE", "TIME", "SECONDS", "ZONE")) {
			// CONVERT UTCLONG time_stamp INTO [DATE dat] [TIME tim [FRACTIONAL SECONDS fs]]
			// [DAYLIGHT SAVING TIME dst] TIME ZONE tz.
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "CONVERT", "TIME", "STAMP")
				&& prevToken.isAnyKeyword("DATE", "TIME")) {
		...
		// - SUM: not considered: this occurs inside AT NEW / AT END OF and changes the
		// wa of the current LOOP
		// - TODO: SET ASSOCIATION not yet considered
		if (firstToken.isKeyword("APPEND") && prevToken.isAnyKeyword("TO", "INTO")) {
			// APPEND {wa | INITIAL LINE | LINES OF jtab [FROM idx1] [TO idx2] [STEP n] } TO
			// itab [SORTED BY comp] ... { ASSIGNING <fs> [CASTING]} | { REFERENCE INTO dref
			// }.
			if (prevToken.isKeyword("TO") && firstToken.matchesOnSiblings(true, "APPEND", "LINES", "OF")
					&& this.matchesDeep(true, TokenSearch.ASTERISK, "TO")) {
			...
			// DELETE itab ...
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "DELETE", "ADJACENT", "DUPLICATES", "FROM")
				&& prevToken.isKeyword("FROM")) {
			// DELETE ADJACENT DUPLICATES FROM itab ...
			return MemoryAccessType.WRITE;
		} else if (firstToken.isKeyword("INSERT") && (prevToken.isKeyword("INTO")
				|| prevPrevToken != null && prevPrevToken.matchesOnSiblings(true, "INTO", "TABLE"))) {
			// INSERT line_spec INTO {{TABLE itab} | {itab INDEX idx} | {itab}} (but NOT
			// 'INSERT dtab FROM TABLE @itab'!)
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "LOOP", "AT") && prevToken.isKeyword("INTO")) {
			// LOOP AT ... { {INTO wa } | { ASSIGNING <fs> [CASTING] [ELSE UNASSIGN] } | {
			// REFERENCE INTO dref }
			return MemoryAccessType.WRITE;
			...
			// READ TABLE ... { {INTO wa } | { ASSIGNING <fs> [CASTING] [ELSE UNASSIGN] } |
			// { REFERENCE INTO dref }
			return MemoryAccessType.WRITE;
			...
			// SORT itab ...
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "DESCRIBE", "FIELD") && prevToken.isAnyKeyword("TYPE",
				"COMPONENTS", "LENGTH", "DECIMALS", "OUTPUT-LENGTH", "HELP-ID", "MASK")) {
			// DESCRIBE FIELD dobj [TYPE typ [COMPONENTS com]] [LENGTH ilen IN
			// {BYTE|CHARACTER} MODE] [DECIMALS dec] [OUTPUT-LENGTH olen] [HELP-ID hlp]
			// [EDIT MASK mask].
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "DESCRIBE", "TABLE")
				&& prevToken.isAnyKeyword("KIND", "LINES", "OCCURS")) {
			// DESCRIBE TABLE itab [KIND knd] [LINES lin] [OCCURS n].
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "DESCRIBE", "DISTANCE", "BETWEEN")
				&& prevToken.isKeyword("INTO")) {
		...
		if (firstToken.matchesOnSiblings(true, "CLOSE", "CURSOR") && prevToken.isKeyword("CURSOR")) {
			// CLOSE CURSOR @dbcur.
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "FETCH", "NEXT", "CURSOR")
				&& prevToken.isAnyKeyword("INTO", "OF", "TABLE", "RESULT")) {
			// FETCH NEXT CURSOR dbcur { { INTO ( elem1, elem2, ...) } | { INTO
			// [CORRESPONDING FIELDS OF] wa [indicators] } | { INTO|APPENDING [CORRESPONDING
			// FIELDS OF] TABLE itab [indicators] [PACKAGE SIZE n] } }
			...
			if (prevToken.isAnyKeyword("INTO", "OF", "TABLE", "RESULT") || isParentPrevInto) {
				// SELECT ... { { INTO ( elem1, elem2, ...) } | { INTO [CORRESPONDING FIELDS OF]
				// wa [indicators] } | { INTO|APPENDING [CORRESPONDING FIELDS OF] TABLE itab
				// [indicators] [PACKAGE SIZE n] } }
				// EXTENDED RESULT @oref
				// WITH: cp.
				// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abapwith.htm
				return MemoryAccessType.WRITE;
			} else if (prevToken.isKeyword("NEW")
					&& (prevPrevToken != null && prevPrevToken.isAnyKeyword("INTO", "OF", "TABLE")
							|| isParentPrevInto)) {
				// SELECT .. .INTO|APPENDING [CORRESPONDING FIELDS OF] [TABLE] { NEW @dref | NEW
				// @DATA(dref) | NEW @FINAL(dref) }
				// SELECT ... INTO ( { NEW @dref1 | NEW @DATA(dref1) | NEW @FINAL(dref1) }, ...
				// )
				// cp.
				// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abapselect_into_target.htm
				return MemoryAccessType.WRITE;
			}
		}

		// Processing External Data: ABAP and HANA
		// see below for CALL DATABASE PROCEDURE ... [EXPORTING p1 = a1 p2 = a2 ...]
		...
		if (firstToken.isKeyword("IMPORT") && (prevToken.textEquals("=") || prevToken.isKeyword("TO"))) {
			...
			// ... } FROM ...
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "IMPORT", "DIRECTORY", "INTO")
				&& prevToken.isAnyKeyword("INTO", "TO")) {
		...
		if (firstToken.matchesOnSiblings(true, "COMMIT", "ENTITIES") && prevToken.isAnyKeyword("FAILED", "REPORTED")) {
			...
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "GET", "PERMISSIONS")
				&& prevToken.isAnyKeyword("RESULT", "FAILED", "REPORTED")) {
			// GET PERMISSIONS ... RESULT result_tab [FAILED failed_resp] [REPORTED
			// reported_resp]
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "MODIFY", "ENTITY|ENTITIES")
				&& prevToken.isAnyKeyword("RESULT", "FAILED", "MAPPED", "REPORTED")) {
			// MODIFY ENTITY, ENTITIES ... [FAILED failed_resp] [REPORTED reported_resp]
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "MODIFY", "ENTITY|ENTITIES")
				&& prevToken.isAnyKeyword("FROM", "WITH")) {
			// MODIFY ENTITY, ENTITIES ... FROM fields_tab / AUTO FILL CID WITH fields_tab /
			// [AUTO FILL CID] FIELDS ( comp1 comp2 ... ) WITH fields_tab / [AUTO FILL CID]
			// SET FIELDS WITH fields_tab:
			// A syntax check on static read-only fields is not possible for all variants,
			// therefore using READ_WRITE to prevent
			// introduction of FINAL for the fields_tab
			return MemoryAccessType.READ_WRITE;
		} else if (firstToken.matchesOnSiblings(true, "READ", "ENTITY|ENTITIES")
				&& prevToken.isAnyKeyword("RESULT", "FAILED", "REPORTED")) {
		...
		if (firstToken.matchesOnSiblings(true, "GET", "PARAMETER") && prevToken.isKeyword("FIELD")) {
			...
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "GET", "LOCALE")
				&& prevToken.isAnyKeyword("LANGUAGE", "COUNTRY", "MODIFIER")) {
		...
		if (firstToken.matchesOnSiblings(true, "GET", "RUN", "TIME") && prevToken.isKeyword("FIELD")) {
			...
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "SYNTAX-CHECK", "FOR")
				&& prevToken.isAnyKeyword("MESSAGE", "LINE", "WORD", "INCLUDE", "OFFSET", "MESSAGE-ID")) {
			// SYNTAX-CHECK FOR itab MESSAGE mess LINE lin WORD wrd [PROGRAM prog]
			// [DIRECTORY ENTRY dir] [WITH CURRENT SWITCHSTATES] [INCLUDE incl] [OFFSET off]
			// [MESSAGE-ID mid].
			return MemoryAccessType.WRITE;
		}
		...
		if (firstToken.matchesOnSiblings(true, "CALL", "TRANSFORMATION")
				&& (prevToken.isKeyword("XML") || prevToken.textEquals("="))
				&& !(isAttached() && prev.textEquals("("))) {
		...
		if ((firstToken.matchesOnSiblings(true, "CALL", "METHOD", "OF")
			...
			// CALL METHOD OF ole meth [= rc] [EXPORTING p1 = f1 p2 = f2 ...] [NO FLUSH]
			// [QUEUE-ONLY].
			// GET PROPERTY OF ole prop = dobj [NO FLUSH] [QUEUE-ONLY] [EXPORTING p1 = f1 p2
			// = f2 ...].
			// token is only in a write position if the keyword EXPORTING is NOT found
			// before it
			Token testToken = prevToken;
		...
		if (firstToken.matchesOnSiblings(true, "GET", "CURSOR")
				&& prevToken.isAnyKeyword("FIELD", "VALUE", "LENGTH", "OFFSET", "LINE", "AREA")) {
			// GET CURSOR { { FIELD field [VALUE val] [LENGTH len] [OFFSET off] [LINE lin]
			// [AREA area] } | { LINE line } }.
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "GET", "PF-STATUS")
				&& prevToken.isAnyKeyword("PF-STATUS", "PROGRAM", "EXCLUDING")) {
			...
			// LOOP AT SCREEN INTO wa (this branch is probably never reached, because it is
			// already handled with LOOP AT ... INTO)
			return MemoryAccessType.WRITE;
		...
		// User Dialogs: Lists
		if (firstToken.matchesOnSiblings(true, "DESCRIBE", "LIST")
				&& prevToken.isAnyKeyword("LINES", "PAGES", "LINE", "PAGE", "LINE-SIZE", "LINE-COUNT", "FIRST-LINE",
						"TOP-LINES", "TITLE-LINES", "HEAD-LINES", "END-LINES", "INDEX")) {
			// DESCRIBE LIST { {NUMBER OF {LINES|PAGES} n} | {LINE linno PAGE page} | {PAGE
			// pagno [LINE-SIZE width] [LINE-COUNT page_lines] [LINES lines]
			// [FIRST-LINE first_line] [TOP-LINES top_lines] [TITLE-LINES title_lines]
			// [HEAD-LINES header_lines] [END-LINES footer_lines] } } [INDEX idx].
			return MemoryAccessType.WRITE;
		} else if (firstToken.matchesOnSiblings(true, "READ", TokenSearch.makeOptional("CURRENT"), "LINE")
				&& prevToken.isKeyword("INTO")) {
			// READ { {LINE line [{OF PAGE page}|{OF CURRENT PAGE}] [INDEX idx]} | {CURRENT
			// LINE} }
		...
		// TODO: further Obsolete Statements missing (except those already covered
		// below)
		...
		if (firstToken.isKeyword("COMPUTE") && prevToken.isAnyKeyword("COMPUTE", "EXACT")) {
			...
			return MemoryAccessType.WRITE;
		} else if (firstToken.isAnyKeyword("MULTIPLY", "DIVIDE", "MULTIPLY-CORRESPONDING", "DIVIDE-CORRESPONDING")
		...
		if (prevToken.textEquals("=") && (firstToken.matchesOnSiblings(true, "CALL", "METHOD|FUNCTION|BADI")
			...
			while (accessKeyword != null && !accessKeyword.isAnyKeyword("EXPORTING", "IMPORTING", "TABLES", "CHANGING",
		...
		if (firstToken.isKeyword("PERFORM")) {
			...
			// if no access keyword is found, we use the default result .READ below
			if (accessKeyword != null) {
				// variables after USING are in a read position, but the syntax check does not
				// prevent them to be changed inside the FORM
	...
	/**
	 * If this Token starts a list of parameters or components, the corresponding
	 * end Token is returned; otherwise null.
	 * 
	 * @return
	 */
	public Token getEndOfParamsOrComponentsList() {
		...
		if (textEqualsAny("boolc(", "boolx(", "xsdbool(")) {
			// skip built-in functions for logical expressions; also skip "boolx( bool =
			// log_exp bit = bit )",
			// because parameters that expect a log_exp are not yet considered in
			// AlignParametersRule.
			return null;

			// By contrast, NO need to skip the other
			// ABAP.builtInFunctionsWithoutNamedParams, because they have the same
			// syntax as a method call "any_method( arg )" that omits the parameter name.
			// After excluding log_exp, the
			// argument could be of type table expression, numeric, string / byte string, or
			// table, e.g.
			// line_index( table_exp ), sin( arg ), strlen( arg ), lines( itab ).
			// Also note that a class may define methods with the same name as a built-in
			// function; in such a case,
			// the built-in function is 'shadowed' (even if the method has a different
			// signature) and the method is called.

		} else if (isSqlOperation && isAnyKeyword("OVER(", "HIERARCHY(", "HIERARCHY_SIBLINGS(", "HIERARCHY_ANCESTORS(",
				"HIERARCHY_DESCENDANTS(", "HIERARCHY_ANCESTORS_AGGREGATE(", "HIERARCHY_DESCENDANTS_AGGREGATE(")) {
			// skip win_func OVER( ... ) and SELECT FROM hierarchy_data( ... )
			return null;

		} else if (isSqlOperation && textEqualsAny(ABAP.abapSqlFunctions)) {
			// skip ABAP SQL table functions, unless they use assignments to formal
			// parameters
			...
			// stand-alone " ( " only starts a component list in case of LOOP AT ... GROUP
			// BY ( ... ) or if it is a row
			// inside a VALUE or NEW constructor expression, not if it is part of arithmetic
			// expressions etc.

			Token prev = getPrevCodeSibling();
			if (parent == null && this.opensGroupKey()) {
				// LOOP AT ... GROUP BY ( key1 = dobj1 key2 = dobj2 ... [gs = GROUP SIZE] [gi =
				// GROUP INDEX] ) ...
				...
				// WHERE ... IN ( SELECT ... ), WHERE ... IN ( ..., ... ), WHERE ... EXISTS (
				// SELECT ... ),
				// WHERE operand {=|...} ALL|ANY|SOME ( SELECT ... ), INTO ( ..., ... )
				return null;
			} else if (parent == null) {
				// otherwise, stand-alone " ( " on top level must be part of an arithmetic
				// expression
				return null;
			...
			// the stand-alone " ( " may still be part of an arithmetic expression, as in
			// "VALUE #( param = a * ( b + c ) )";
			// such brackets must be preceded by " = " or an arithmetic operator or by
			// keywords like
			// - "UNTIL" / "WHILE" (see
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abenfor_conditional.htm)
			// or
			// - "WHERE (see
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abenfor_conditional.htm)
			// The above conditions already excluded the case " ( ( ".
			// do NOT use getPrevCodeToken() here, as it may return the "#(" of the VALUE or
			// NEW constructor, which has TokenType.OTHER_OP
			if (prev != null && !prev.textEquals(")") && (prev.isAssignmentOperator() || prev.isComparisonOperator()
					|| prev.isOtherOp() || prev.isAnyKeyword("UNTIL", "WHILE", "WHERE"))) {
				return null;
			}

		} else {
			// as the token opens a level but is not a stand-alone "(", it must be a method
			// call or a constructor expression,
			// because the Tokens "method_name(", "VALUE #(" etc. have text immediately
			// attached to the top level parenthesis
			...
			if (prev != null && prev.isAnyKeyword("NEW", "VALUE")) {
				...
				// do not align inside these constructor expressions (exception: NEW and VALUE,
				// see above)
				return null;
			}
		}
	...
	public boolean opensGroupKey() {
		...
		return prevPrev != null && prevPrev.isKeyword("GROUP") && prev.isKeyword("BY")
				&& parentCommand.firstCodeTokenIsKeyword("LOOP");
	}

	/**
	 * If this Token starts a logical expression, the last code Token of the logical
	 * expression is returned; otherwise null.
	 * 
	 * @return
	 */
	public Token getLastTokenOfLogicalExpression() {
		...
		if (this == firstCode && firstCode.isAnyKeyword("IF", "ELSEIF", "CHECK", "WHILE")) {
		...
		} else if (isKeyword("WHERE") && parent == null
				&& firstCode.matchesOnSiblings(true, "LOOP AT|MODIFY|DELETE|FOR")
			...
			// WAIT FOR ASYNCHRONOUS TASKS [MESSAGING CHANNELS] [PUSH CHANNELS] UNTIL
			// log_exp [UP TO sec SECONDS].
		...
		if (ctorKeyword == null || !ctorKeyword.isKeyword()) {
			...
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abenfor.htm,
			// context:
			...
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abenfor_conditional.htm,
			// context:
			// - REDUCE|NEW|VALUE identifier( ... FOR var = rhs [THEN expr] UNTIL|WHILE
			// log_exp [let_exp] ... )
			...
			// [...]] ) ...
			// FILTER type( itab [EXCEPT] IN ftab [USING KEY keyname] WHERE c1 op f1 [AND c2
			// op f2 [...]] ) ...
			return parent.getNextSibling().getPrevCodeToken();

		} else if (isKeyword("WHERE") && ctorKeyword.isAnyKeyword("REDUCE", "NEW", "VALUE")
				&& getNextCodeSibling() != null && getNextCodeSibling().textEquals("(")) {
			// "FOR, Table Iterations",
			// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/index.htm?file=abenfor_itab.htm
			// Context:
			// - VALUE|NEW type( [let_exp] [BASE itab] [FOR for_exp1 FOR for_exp2 ... ] (
			// line_spec1 ) ( line_spec2 ) ... ) ...
			// - REDUCE type( [let_exp] INIT ... FOR for_exp1 FOR for_exp2 ... NEXT ... )
			// FOR clause:
			// - FOR wa|<fs> IN itab [INDEX INTO idx] [ cond] [let_exp]
			// - FOR GROUPS [group|<group>] OF wa|<fs> IN itab [INDEX INTO idx] [cond] GROUP
			// BY group_key [ASCENDING|DESCENDING [AS TEXT]] [WITHOUT MEMBERS] [let_exp] ...
			// - FOR { wa|<fs> IN GROUP group [INDEX INTO idx] [ WHERE ( log_exp )] }
			// | { GROUPS OF wa|<fs> IN GROUP group [INDEX INTO idx] [ WHERE ( log_exp )]
			// GROUP BY group_key [ASCENDING|DESCENDING [AS TEXT]] [WITHOUT MEMBERS] } [
			// let_exp] ...
			// Condition [cond]: in this case, the condition is in parentheses:
			// - ... [USING KEY keyname] [FROM idx1] [TO idx2] [STEP n] [WHERE ( log_exp
			// )|(cond_syntax)] ...
		...
		if (parentCommand.isAbapSqlOperation()) {
			if (isKeyword("WHEN")) {
				...
				// find the next keyword that does NOT fit into a sql condition (but instead
				// starts a new clause, join, case etc.)
				Token end = getNextCodeSibling();
				while (end != null && !end.isPeriod()) {
					if (!end.isKeyword()) {
						// the condition can only end with a keyword, a period or a closing parenthesis
						// (see while condition above)
						...
						// however, EQ, NE etc. as well as BETWEEN, IN, LIKE should be categorized as
						// TokenType.COMPARISON_OP
					} else if (end.isAnyKeyword("ALL", "ANY", "SOME")) {
						// operand1 {=|EQ|<>|NE|>|GT|<|LT|>=|GE|<=|LE} [ALL|ANY|SOME] ( SELECT
						// subquery_clauses ... )
					} else if (end.isAnyKeyword("NULL", "INITIAL", "ESCAPE")) {
						// operand IS [NOT] NULL, operand IS [NOT] INITIAL, operand1 [NOT] LIKE operand2
						// [ESCAPE esc]
						...
						// if constructor operators could ever be used, they would be part of the
						// logical expression
					} else {
						break;
					}
					// the logical expression may be at the end of parentheses,
					// e.g. the ON condition in 'SELECT FROM ( dtab1 AS t1 INNER JOIN dtab2 AS t2 ON
					// t2~d = t1~d ) ...'
					if (end.getNextCodeSibling() == null)
						return end;
					end = end.getNextCodeSibling();
				}
				// check whether the end meets the expectations, otherwise ignore this logical
				// expressions by returning null
				if (end != null && !end.isPeriod()) {
					if (end.startsDdlUnionEtc()) {
						// end of SELECT
					} else if (end.isAnyKeyword("FIELDS", "FOR", "WHERE", "GROUP", "HAVING", "ORDER", "INTO",
							"APPENDING")) {
						// next SQL clause
					} else if (end.isAnyKeyword("USING", "UP", "%_HINTS", "PRIVILEGED", "OFFSET", "BYPASSING",
							"CONNECTION")) {
					...
					} else if (end.isAnyKeyword("SOURCE", "CHILD", "PERIOD", "SIBLINGS", "DEPTH", "MULTIPLE", "ORPHANS",
							"CYCLES", "LOAD", "GENERATE", "DISTANCE", "MEASURES", "WHERE", "WITH")) {
						// additions of HIERARCHY( ... ),
						// HIERARCHY_DESCENDANTS|HIERARCHY_ANCESTORS|HIERARCHY_SIBLINGS( ... ),
						// HIERARCHY_ANCESTORS_AGGREGATE( ... )
					} else {
						if (Program.showDevFeatures())
							throw new IllegalArgumentException("Unexpected end '" + end.getText()
									+ "' of SQL condition: " + parentCommand.toString());
						// ignore this unexpected syntax by returning null as if no logical expression
						// was found
						end = null;
					}
				}
	...
	/**
	 * If this Token starts a logical expression in DDL, the last code Token of the
	 * logical expression is returned; otherwise null.
	 * 
	 * @return
	 */
	public Token getLastTokenOfDdlLogicalExpression() {
		...
		if (isKeyword("WHEN")) {
			...
			return (thenToken == null) ? null : thenToken.getPrevCodeToken();
				...
				&& (textEquals(DDL.COLON_SIGN_STRING) && getNextSibling() != null
						&& !getNextSibling().isAnyKeyword("INNER", "LEFT", "WHERE")
						|| isAnyKeyword("INNER", "OUTER") && getNextSibling() != null
								&& !getNextSibling().isKeyword("WHERE")
						|| isKeyword("WHERE"))) {
			...
			return parent.getNextCodeSibling().getPrevCodeToken();
				...
				|| isKeyword("ON") && (prevCode == null || !prevCode.isKeyword("PROJECTION")) // exclude "AS PROJECTION
																								// ON"
				|| isKeyword("FILTER") && prevCode != null && prevCode.isKeyword("DEFAULT")) { // "ASSOCIATION ... WITH
																								// DEFAULT FILTER"

			// find the next keyword that does NOT fit into a DDL condition (but instead
			// starts a new clause, join, case, list element etc.)
			Token end = getNextCodeSibling();
			while (end != null) {
				if (end.textEqualsAny(DDL.BRACE_OPEN_STRING, DDL.COMMA_SIGN_STRING, DDL.SEMICOLON_SIGN_STRING)) {
					...
					// the condition can only end with a keyword or with , ; { (see while condition
					// above)
				} else if (end.isAnyKeyword("NOT", "AND", "OR")) {
					// sql_cond -> rel_exp | [NOT] sql_cond [AND|OR sql_cond] ...
				} else if (end.isAnyKeyword("IS", "BETWEEN", "IN", "LIKE")) {
					...
					// aggregate functions are only allowed in HAVING (and WHEN ... THEN), but not
					// in WHERE / ON / DEFAULT FILTER,
					// cp.
					// https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abencds_cond_expr_operands_v2.htm
				} else {
					break;
				}
				end = end.getNextCodeSibling();
			...
			if (end != null) {
				if (end.textEqualsAny(DDL.BRACE_OPEN_STRING, DDL.COMMA_SIGN_STRING, DDL.SEMICOLON_SIGN_STRING)) {
					...
					// next SELECT clause
				} else if (end.isAnyKeyword("INNER", "LEFT", "RIGHT", "JOIN", "CROSS", "EXACT", "MANY", "ONE", "TO",
						"ASSOCIATION", "COMPOSITION")) {
					// next join or association
				} else if (end.isAnyKeyword("WITH")) {
					// ASSOCIATION [cardinality] TO target [AS assoc] ON cds_cond [WITH DEFAULT
					// FILTER cds_cond]
				} else {
					if (Program.showDevFeatures())
						throw new IllegalArgumentException(
								"Unexpected end '" + end.getText() + "' of DDL condition: " + parentCommand.toString());
					// ignore this unexpected syntax by returning null as if no logical expression
					// was found
					end = null;
				}
	...
	public boolean startsFunctionalMethodCall(boolean considerBuiltInFunctionsAsMethodCalls) {
		...
		// if applicable, exclude built-in functions (which do NOT set sy-subrc, as
		// opposed to ENDMETHOD in functional calls)
	...
	/**
	 * Returns true if this Token represents a type (dtype, abap_type, enum_type,
	 * mesh_type, ref_type, struct_type, table_type), not a data object (dobj). This
	 * could either be in the definition with TYPES type ... (this is only checked
	 * for includeTypeDef == true), or when the type is used for declaring a data
	 * object (e.g. DATA ... TYPE type) or another type (e.g. TYPES ... TYPE
	 * STANDARD TABLE OF type) or after a constructor operator (e.g. ... = VALUE
	 * type( ... ))
	 */
	public boolean isTypeIdentifier(boolean includeTypeDef) {
		...
		// if the identifier is preceded by keywords, determine the first of these
		// keywords (skipping chain colons)
		...
		if (includeTypeDef && parentCommand.firstCodeTokenIsKeyword("TYPES")) {
			if (firstKeyword != null && firstKeyword.isKeyword("TYPES")) {
				// the Token is the identifier directly after TYPES [:] [BEGIN OF|BEGIN OF
				// ENUM|BEGIN OF MESH]
				return true;
		...
		// b) usage of a type
		// - in a constructor expression, e.g. VALUE type( ), COND type( ), or any other
		// constructor operator
		if (this.textEndsWith("(") && prevSibling != null && prevSibling.isAnyKeyword(ABAP.constructorOperators)) {
			return true;
		}

		// - in a declaration, e.g. DATA dobj TYPE STANDARD TABLE OF REF TO type
		// (including declaration of a dependent type with TYPES)
		if (parentCommand.isDeclaration() || parentCommand.isDeclarationInClassDef()) { // CONSTANTS, DATA,
																						// FIELD-SYMBOLS, TYPES,
																						// CLASS-DATA, STATICS
			// the identifier represents a type if it is preceded by a sequence of keywords
			// that starts with TYPE (NOT with LIKE)
			return (firstKeyword != null && firstKeyword.isKeyword("TYPE"));
		} else if (parentCommand.firstCodeTokenIsKeyword("INCLUDE")) {
			// the identifier represents a type if it is preceded by "INCLUDE TYPE" (NOT by
			// "INCLUDE STRUCTURE", which is followed by a data object)
			...
			// CATCH [BEFORE UNWIND] cx_class1 cx_class2 ... [INTO oref].
			// for DATA(lx_any), prevSibling will be null (so it is excluded); for
			// cx_class2, firstKeyword will be null;
	...
	public boolean canInsertStressTestTokenAfter(StressTestType stressTestType) {
		// nothing can be inserted between attached tokens like DATA(lv_any) or
		// lv_text+5(lv_length)
		if (next != null && next.isAttached() && !next.isCommaOrPeriod() && !next.isChainColon())
			return false;

		/*
		 * // do not insert a comment, pragma (or even line break) between
		 * "OPEN CURSOR", otherwise RND Parser will show all // commas in the SELECT
		 * field list as erroneous if (isKeyword("OPEN") && next != null &&
		 * next.isKeyword("CURSOR")) return false;
		 */

		// if a pragma follows on the same line after a period, it must be considered as
		// the "nextToken" here
		...
		case LINE_END_COMMENT:
			return !isPragmaOrComment() && (next == null || !next.isPragmaOrComment())
					&& (nextCommandToken == null || !nextCommandToken.isPragma());

		case COMMENT_LINE:
			return !isPragma() && (next != null) && !next.isPragmaOrComment()
					&& (nextCommandToken == null || !nextCommandToken.isPragma());
	...
	private Token insertFirstChild(Token newToken) throws IntegrityBrokenException {
		...
		// since 'this' Token opens a level, it cannot be a comment; however, newToken
		// may be:
		if (newToken.isComment() && next != null && next.lineBreaks == 0) {
			next.ensureLineBreak();
		}
	...
	public boolean isSqlTypeInCast() {
		return textEqualsAny(ABAP.abapSqlLiteralTypes) && parent != null && parent.textEquals("CAST(")
				&& getPrevCodeSibling() != null && getPrevCodeSibling().isKeyword("AS");
	}
	...
	public boolean condenseUpTo(Token last, int maxLineLength, int indent, boolean keepCommentLines) {
		...
		while (token != null) {
			// determine whether the Token is (or should be) attached; otherwise, it can be
			// moved to the next line
			int spacesLeft = (token.isAttached() || token.isCommaOrPeriod() || token.isChainColon()) ? 0 : 1;
			if (spacesLeft > 0)
				lastMovableToken = token;

			// if needed, move Token(s) to the next line, otherwise move it directly behind
			// the previous Token
			boolean isLineEndComment = parentCommand.isDdlOrDcl() ? token.isDdlLineEndComment()
					: token.isQuotMarkCommentLine();
	...
	/**
	 * returns true if this Token is the NEW keyword in SELECT ... INTO(NEW ...) and
	 * is attached to the opening parenthesis
	 */
	...
	/**
	 * returns a StrucInfo instance if the Token as a whole is the name of a
	 * structure or one of its components; for asterisk comments, only the most
	 * frequent case of a commented-out assignment inside a VALUE or NEW constructor
	 * is considered
	 */
	public StrucElement getStrucInfo() {
		...
		Token prevCode = getPrevCodeSibling();
		if (isIdentifier && parent != null && parent.textEndsWith("[") && nextCode != null
				&& nextCode.textEquals("=")) {
		...
		if (parent != null && parent.textEndsWith("(")) {
			...
			if (ctorStart != null && (ctorStart.isIdentifier() || ctorStart.textEquals("#("))) {
				...
				if (ctorKeyword == null) {
					...
					// FILTER type( itab [EXCEPT] [IN ftab] [USING KEY keyname] WHERE c1 op f1 [AND
					// c2 op f2 [...]] ) ...
					...
					// determine whether this Token is part of a MAPPING ... assignment, possibly
					// inside parentheses: MAPPING ( t1 = s1 ... )
					Token prevKeyword2 = (prevKeyword == null && parent.textEquals("(")) ? parent : prevKeyword;
					while (prevKeyword2 != null && (!prevKeyword2.isKeyword()
							|| prevKeyword2.isAnyKeyword("DEFAULT", "DISCARDING", "DUPLICATES")))
		...
		// for comments, only the most frequent case of a commented-out assignment
		// inside a VALUE or NEW constructor is considered
		if (isComment())
			return null;
		...
		// exclude the case of "KEY keyname COMPONENTS" for multiple of the following
		// cases
		...
		if (parentCommand.isAbapSqlOperation()) {
			if (text.indexOf(ABAP.TILDE) >= 0) {
				...
				// it was already checked above whether the text contains no
				// ABAP.COMPONENT_SELECTOR, e.g. '@ls_any-comp'
				return null;
				...
				if (parent != null && parent.getPrevCodeSibling() != null
						&& parent.getPrevCodeSibling().isKeyword("FROM")) {
					// for parameters, use the view name (i.e. this.parent) as the parent Token, not
					// firstCode, so they form a context
					// of their own: thus, if no CamelCase is known for the parameters, they will
					// not prevent the field names from being changed
					return StrucElement.createForParameterName(this, parent);
				} else {
					return StrucElement.createForComponentName(this, firstCode);
				}

			} else if (prevCode != null && (prevCode.isComparisonOperator() || prevCode.isAssignmentOperator())
					&& textEqualsAny(ABAP.ABAP_TRUE, ABAP.ABAP_FALSE, ABAP.ABAP_SPACE)) {
				return null;
		...
		// Internal Tables
		if (firstCode.isKeyword("APPEND") && prevPrevKeyword != null && prevPrevKeyword.isKeyword("SORTED")
				&& prevKeyword.isKeyword("BY")) {
			// APPEND ... TO itab SORTED BY comp.
			return StrucElement.createForComponentName(this, firstCode);

		} else if (firstCode.matchesOnSiblings(true, "DELETE|READ", "TABLE") && nextCode != null
				&& nextCode.textEquals("=") && prevKeyword != null && prevKeyword.isAnyKeyword("KEY", "COMPONENTS")) {
			// DELETE TABLE itab WITH TABLE KEY [keyname COMPONENTS] {comp_name1|(name1)} =
			// operand1 ...
			// READ TABLE itab WITH [TABLE] KEY [keyname COMPONENTS] {comp_name1|(name1)} =
			// operand1 ...
			return textEquals(ABAP.TABLE_LINE) ? null : StrucElement.createForComponentName(this, prevKeyword);

		} else if (firstCode.matchesOnSiblings(true, "READ", "TABLE") && prevKeyword != null
				&& prevKeyword.isAnyKeyword("COMPARING", "TRANSPORTING")) {
			// READ TABLE itab INTO ... [COMPARING { { comp1 comp2 ...}|{ALL FIELDS}|{NO
			// FIELDS} }] [TRANSPORTING { { comp1 comp2 ...}|{ALL FIELDS} }] ...
			return StrucElement.createForComponentName(this, firstCode);

		} else if (firstCode.matchesOnSiblings(true, "LOOP", "AT") && nextCode != null
				&& (nextCode.isComparisonOperator() || nextCode.isKeyword("IS"))) {
			// LOOP AT itab ... WHERE comp1 = ... AND comp2 IS ... AND comp3 OP ...
			// LOOP AT GROUP ... INTO FINAL(wa) WHERE comp1 = ... - no need to consider
			// 'GROUP BY ( key1 = wa-comp1 key2 = wa-comp2 ... indx = ... size = ... )'
			return textEquals(ABAP.TABLE_LINE) ? null : StrucElement.createForComponentName(this, firstCode);

		} else if (firstCode.matchesOnSiblings(true, "AT", "NEW")
				|| firstCode.matchesOnSiblings(true, "AT", "END", "OF")) {
		...
		if (parentCommand.isDeclaration() || parentCommand.isDeclarationInClassDef()) { // CONSTANTS, DATA,
			...
			if (prevKeyword != null && prevKeyword.isAnyKeyword("KEY", "COMPONENTS")) {
				...
				return StrucElement.createForComponentName(this, prevKeyword); // do not use firstCode here, because the
																				// declaration might be chained
	...
	public boolean startsMultiLineDdlComment() {
		return parentCommand.isDdlOrDcl() && isCommentLine() && textStartsWith(DDL.ASTERISK_COMMENT_START)
				&& !endsMultiLineDdlComment();
	}
	...
	public boolean isDdlInlineComment() {
		return parentCommand.isDdlOrDcl() && textStartsWith(DDL.ASTERISK_COMMENT_START)
				&& textEndsWith(DDL.ASTERISK_COMMENT_END);
	}

	public boolean isCommentedOutDdlAnnotation() {
		if (parentCommand.isDdlOrDcl() && isCommentLine()
				&& textStartsWithAny(DDL.LINE_END_COMMENT, DDL.LINE_END_MINUS_COMMENT)) {
	...
	public ArrayList<Command> setWhitespaceInclAttachedComments(int newLineBreaksMin, int newLineBreaksMax,
			int newSpacesLeft, boolean includeDdlAnnoComments) {
		ArrayList<Command> changedCommands = new ArrayList<>();

		if (prev == null && lineBreaks == 1 && parentCommand.getPrev() != null
				&& parentCommand.getPrev().isCommentLine()) {
			// move to the first attached comment Command (possibly except for commented-out
			// annotations)
			...
			while (testCommand.getFirstTokenLineBreaks() == 1) {
				...
				if (prev == null || !prev.isCommentLine()
						|| !includeDdlAnnoComments && prev.isCommentedOutDdlAnnotation())
					break;
				comment = prev;
				testCommand = prev;
			}

			// set the supplied spacesLeft to all attached Commands, but the supplied
			// lineBreaks only to the first one
			...
			if (comment != null) {
				...
				do {
					...
					int newLineBreaks = determineLineBreaks(isFirst, comment.getFirstTokenLineBreaks(),
							newLineBreaksMin, newLineBreaksMax);
					...
					if (comment.startsMultiLineDdlComment()) {
						// change indent of the remaining Commands that belong to this multi-line
						// comment
						while (!comment.endsMultiLineDdlComment()) {
							...
							if (commentToken.setWhitespace(commentToken.lineBreaks,
									commentToken.spacesLeft + addIndent)) {
								changedCommands.add(comment);
							}
						}
					}
			...
			// move to the first attached comment Token (possibly except for commented-out
			// annotations)
			...
			// set the supplied spacesLeft to all attached Tokens, but the supplied
			// lineBreaks only to the first one
			boolean isFirst = true;
			boolean changed = false;
			if (commentToken != null) {
				do {
					...
					int newLineBreaks = determineLineBreaks(isFirst, commentToken.lineBreaks, newLineBreaksMin,
							newLineBreaksMax);
					...
					if (commentToken.startsMultiLineDdlComment()) {
						...
						while (!commentToken.endsMultiLineDdlComment()) {
							...
							if (commentToken == null || !commentToken.isCommentLine()) // pro forma
								break;
							changed |= commentToken.setWhitespace(commentToken.lineBreaks,
									commentToken.spacesLeft + addIndent);
						}
					}
	...
	public boolean startsDdlJoin() {
		...
		return isAnyKeyword("INNER", "LEFT", "RIGHT", "CROSS", "JOIN") || matchesOnSiblings(true, "EXACT", "ONE", "TO")
				|| matchesOnSiblings(true, "MANY|ONE", "TO") || matchesOnSiblings(true, "TO", "ONE|EXACT|MANY");
	}
	...
	/**
	 * return true if the Token is a colon ":" that belongs to a parameter name like
	 * ":P_Any" (only possible in DDIC-based views)
	 */
	public boolean isDdlParameterColon() {
		if (textEquals(DDL.COLON_SIGN_STRING) && next != null && next.isAttached() && next.isIdentifier()) {
			// in cases like 'select from I_Any(P_Any::P_Other)', only the second colon
			...
			// if the colon is detached from the previous Token, it probably starts a
			// parameter name; since this cannot be
			// safely determined without the syntax of the context, we rather assume it
			// does, so it is not detached
			return !isAttached();
	...
	/**
	 * returns true if this Token represents the definition (but not usage) of an
	 * entity parameter, or the formal parameter of a data source
	 */
	...
	public boolean isDdlTypeName() {
		...
		} else if (parentPrev != null && parentPrev.isKeyword("CAST") && prevCode != null && prevCode.isKeyword("AS")
				&& getNextCodeSibling() == null) {
	...
	/**
	 * Returns true if this Token can be followed by an arithmetic operator. Note
	 * that the '-' for negative numbers in NOT considered an arithmetic operator
	 * here.
	 */
	...
	/**
	 * check whether the Token is an asterisk comment line in the format "***
	 * parameter_or_component = ..." (with any number of spaces and asterisks); if
	 * so, returns a String[4], e.g. { "***", "parameter_or_component_name", "=",
	 * "any_term" }
	 */
	public String[] getBitsOfCommentedOutAssignment() {
		...
		// check whether the asterisk comment line has the format "*** parameter = ..."
		// (with any number of spaces and asterisks)
		...
		return new String[] { asterisksAndParamName[0], asterisksAndParamName[1], "=",
				commentText.substring(equalsSignPos + 1).trim() };
	}
}
