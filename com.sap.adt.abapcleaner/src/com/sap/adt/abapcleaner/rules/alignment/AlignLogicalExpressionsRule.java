...
public class AlignLogicalExpressionsRule extends RuleForLogicalExpressions {
   ...
   public String getExample() {
      return "" 
			...
			+ LINE_SEP + "      FIELDS t1~any_col,"
			+ LINE_SEP + "             t2~other_col"
			+ LINE_SEP + "      WHERE t1~status = @lc_status_active"
			+ LINE_SEP + "           AND t1~any_flag = @abap_false"
			+ LINE_SEP + "          AND ( t2~year > @lv_year_from"
			+ LINE_SEP + "           OR t2~year = @lv_year_from AND t2~month >= @lv_month_from )"
			+ LINE_SEP + "            AND t1~other_col IN ( SELECT other_col FROM third_dtab WHERE third_col = @lv_any_value"
			+ LINE_SEP + "            OR third_col = @lv_other_value )"
			+ LINE_SEP + "      INTO CORRESPONDING FIELDS OF TABLE @lts_any_table."
			+ LINE_SEP + ""
			+ LINE_SEP + "    SELECT FROM demo_expressions"
			+ LINE_SEP + "      FIELDS char1 && '_' && char2        AS group,"
			+ LINE_SEP + "             MIN( num1 ) + MIN( num2 )    AS min,"
			+ LINE_SEP + "             MAX( num1 ) + MAX( num2 )    AS max,"
			+ LINE_SEP + "             CASE WHEN MIN( num1 )  < 0"
			+ LINE_SEP + "              OR MIN( num2 ) < 0"
			+ LINE_SEP + "                  THEN 'X' END            AS negative"
			+ LINE_SEP + "      GROUP BY char1, char2"
			+ LINE_SEP + "      HAVING MIN( num1 ) * MIN( num2 ) > 25"
			+ LINE_SEP + "             AND MAX( num1 ) * MAX( num2 ) < 1000"
			+ LINE_SEP + "      ORDER BY group"
			+ LINE_SEP + "      INTO TABLE @FINAL(grouped_having)."
			+ LINE_SEP + "  ENDMETHOD.";
   }
