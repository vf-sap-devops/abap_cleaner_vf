...
public class AlignParametersRule extends RuleForCommands {
	...
	public enum Columns {
		// LET ... IN expressions
		LET_KEYWORD, LET_PARAMETER, LET_ASSIGNMENT_OP, LET_EXPRESSION, LET_IN_KEYWORD, // only used if "IN" is at the
																						// beginning of a line
																						// (otherwise, "IN" simply stays
																						// after the last
																						// LET_EXPRESSION)
		// keywords like EXPORTING, IMPORTING, ..., COMPONENTS
		KEYWORD,
		// assignments
		PARAMETER, ASSIGNMENT_OP, EXPRESSION;
	...
	public enum ContentType {
		FUNCTIONAL_CALL_PARAMS, PROCEDURAL_CALL_PARAMS, IMPORT_OR_EXPORT, TABLE_KEY, GROUP_KEY, CONSTRUCTOR_EXPR,
	...
	private final static RuleReference[] references = new RuleReference[] {
			...
			new RuleReference(RuleSource.ABAP_STYLE_GUIDE, "Keep parameters behind the call",
					"#keep-parameters-behind-the-call"),
			new RuleReference(RuleSource.ABAP_STYLE_GUIDE, "If you break, indent parameters under the call",
					"#if-you-break-indent-parameters-under-the-call"),
			new RuleReference(RuleSource.ABAP_STYLE_GUIDE, "Line-break multiple parameters",
					"#line-break-multiple-parameters"),
			new RuleReference(RuleSource.ABAP_STYLE_GUIDE, "Stick to a reasonable line length",
			...
			new RuleReference(RuleSource.ABAP_STYLE_GUIDE, "Indent in-line declarations like method calls",
	...
	@Override
	public RuleID getID() {
		return RuleID.ALIGN_PARAMETERS;
	}
	...
	public RuleID[] getDependentRules() {
		return new RuleID[] { RuleID.ALIGN_LOGICAL_EXPRESSIONS };
	}
	...
	public String getExample() {
		return "" + LINE_SEP + "  METHOD align_parameters." + LINE_SEP + "    lts_table = cl_any_class=>create_table("
				+ LINE_SEP + "      EXPORTING" + LINE_SEP + "        iv_contract_id = lo_contract1->ms_data-contract_id"
				+ LINE_SEP + "        iv_contract_type = if_any_interface=>co_any_contract_type" + LINE_SEP
				+ "        iv_item_type = lo_item->get_item_data( )-item_type )." + LINE_SEP + LINE_SEP
				+ "    \" commented-out parameters are aligned, too:" + LINE_SEP
				+ "    lts_other_table = cl_other_class=>create_table(" + LINE_SEP + "      EXPORTING" + LINE_SEP
				+ "        iv_item_key = '12345'" + LINE_SEP + "        iv_category = 'ABC'" + LINE_SEP
				+ "*        iv_size = 100'" + LINE_SEP + "        iv_name = 'ANY_NAME'" + LINE_SEP
				+ "        iv_qty = 8 )." + LINE_SEP + LINE_SEP + "    CALL METHOD procedural_call_example" + LINE_SEP
				+ "      EXPORTING" + LINE_SEP + "        iv_contract_id = lo_contract1->ms_data-contract_id" + LINE_SEP
				+ "        iv_item_key = '13579'" + LINE_SEP + "      IMPORTING" + LINE_SEP
				+ "        ev_category = lv_any_category" + LINE_SEP + "        ev_item_type = lv_any_item_type"
				+ LINE_SEP + "      CHANGING" + LINE_SEP + "        cv_qty = lv_quantity." + LINE_SEP + LINE_SEP
				+ "    ets_table = VALUE #( date = gc_any_date" + LINE_SEP + "                           id = gc_any_id"
				+ LINE_SEP + LINE_SEP + "                           ( item_name = 'ANY'" + LINE_SEP
				+ "*                      size = 'M'" + LINE_SEP + "                              quantity = 1 )"
				+ LINE_SEP + "                             ( item_name = 'OTHER'" + LINE_SEP
				+ "                                quantity = 2" + LINE_SEP
				+ "                                reference_id = '12345' )" + LINE_SEP
				+ "                                   ( item_name = 'THIRD'" + LINE_SEP
				+ "                                  quantity = 3 ) )." + LINE_SEP + LINE_SEP
				+ "    ets_fulfillment = VALUE #( event_date = gc_any_event_date" + LINE_SEP
				+ "                                amount = gc_any_amount" + LINE_SEP
				...
				+ LINE_SEP + LINE_SEP + "    READ TABLE lt_any_table_name ASSIGNING <ls_table_row>" + LINE_SEP
				+ "         WITH KEY field1 = ls_any_structure-field1" + LINE_SEP
				+ "                  fld2 = ls_any_structure-fld2" + LINE_SEP
				+ "                  long_field_name3 = ls_any_structure-long_field_name_3." + LINE_SEP + LINE_SEP
				+ "    result = VALUE #( BASE result ( id = 1 name = 'abc' ) )." + LINE_SEP + LINE_SEP
				+ "    LOOP AT lt_table ASSIGNING <fs>" + LINE_SEP
				+ "         GROUP BY ( key1 = <fs>-any_component key2 = get_value( <fs>-other_component )" + LINE_SEP
				+ "                     indx = GROUP INDEX count = GROUP SIZE )" + LINE_SEP
				+ "         ASSIGNING FIELD-SYMBOL(<group>)." + LINE_SEP + LINE_SEP
				+ "      cl_demo_output=>write( |{ <group>-indx } { <group>-key1 } { <group>-key2 } { <group>-count }| )."
				+ LINE_SEP + "    ENDLOOP." + LINE_SEP + "  ENDMETHOD.";
	}

	final ConfigIntValue configMaxLineLength = new ConfigIntValue(this, "MaxLineLength",
			"Maximum line length A (normal)", "", MIN_LINE_LENGTH_ABAP, DEFAULT_LINE_LENGTH_ABAP, ABAP.MAX_LINE_LENGTH);
	final ConfigIntValue configMaxLineLengthForSingleLine = new ConfigIntValue(this, "MaxLineLengthForSingleLine",
			"Maximum line length B (for tabular style)", "", MIN_LINE_LENGTH_ABAP, HIGHEST_DEFAULT_LINE_LENGTH_ABAP,
			ABAP.MAX_LINE_LENGTH);
	final ConfigIntValue configMaxParamCountBehindProceduralCall = new ConfigIntValue(this,
			"MaxParamCountBehindProceduralCall", "Procedural call: continue behind the call for up to", "parameters", 0,
			0, 100);
	final ConfigIntValue configMaxParamCountBehindFunctionalCall = new ConfigIntValue(this,
			"MaxParamCountBehindFunctionalCall", "Functional call: continue behind the call for up to", "parameters", 0,
			100, 100);
	final ConfigBoolValue configPutProceduralCallKeywordsOnOwnLine = new ConfigBoolValue(this,
			"PutProceduralCallKeywordsOnOwnLine", "Procedural call: put keywords (EXPORTING etc.) on own line", false);
	final ConfigBoolValue configPutFunctionalCallKeywordsOnOwnLine = new ConfigBoolValue(this,
	...
	final ConfigEnumValue<ComponentsOnSingleLine> configKeepComponentsOnSingleLine = new ConfigEnumValue<ComponentsOnSingleLine>(
			this, "KeepParametersOnSingleLine", "Table rows: Keep multiple components on single line",
			new String[] { "never", "if maximum line length B is observed", "always" }, ComponentsOnSingleLine.values(),
			ComponentsOnSingleLine.IF_BELOW_MAX_LINE_LENGTH);
	final ConfigEnumValue<ComponentsOnSingleLine> configKeepOtherOneLiners = new ConfigEnumValue<ComponentsOnSingleLine>(
			this, "KeepOtherOneLiners", OPTION_NAME_KEEP_OTHER_ONE_LINERS,
			new String[] { "never", "if maximum line length A is observed", "always" }, ComponentsOnSingleLine.values(),
			ComponentsOnSingleLine.NEVER, ComponentsOnSingleLine.NEVER, LocalDate.of(2024, 1, 1));
	final ConfigEnumValue<ContentLeftOfAssignOp> configAllowContentLeftOfAssignOp = new ConfigEnumValue<ContentLeftOfAssignOp>(
			this, "AllowContentLeftOfAssignOp", "Allow line starts left of assignment operator",
			new String[] { "never", "only to keep maximum line length", "always" }, ContentLeftOfAssignOp.values(),
			ContentLeftOfAssignOp.TO_KEEP_MAX_LINE_LENGTH, ContentLeftOfAssignOp.NEVER, LocalDate.of(2022, 3, 19));
...
//	}
	...
	protected boolean executeOn(Code code, Command command, int releaseRestriction)
		...
		// note that cases processed here should be skipped in the
		// AlignWithSecondWordRule
		// (exception: RAISE ... MESSAGE ... EXPORTING ..., where the MESSAGE section
		// benefits from AlignWithSecondWordRule)

		// align CALL METHOD|FUNCTION|BADI without parentheses (a call with parentheses
		// will simply be handled by the "all other cases" section below)
		// also align CREATE OBJECT, if it was not replaced by a NEW constructor by the
		// CreateObjectRule,
		// and RECEIVE RESULTS FROM FUNCTION func [KEEPING TASK] (with IMPORTING,
		...
		if (firstCode.matchesOnSiblings(true, "CALL", "METHOD|FUNCTION|BADI")
			...
			// CALL FUNCTION has a lot of potential parameters, so we just search the
			// siblings(!) for the first keyword.
			// In case of CALL METHOD identifier( ... ), this is deliberately NOT found
			// among the siblings
			// (see ABAP Reference: "CALL FUNCTION - Quick reference")
			Token firstKeyword = firstCode.getLastTokenOnSiblings(true, TokenSearch.ASTERISK,
					"EXPORTING|IMPORTING|TABLES|CHANGING|RECEIVING|EXCEPTIONS");
			if (firstKeyword != null && firstKeyword.getParent() == null) { // otherwise, continue below
				...
				if (alignParams(code, command, parentToken, period, baseIndent, baseIndent,
						ContentType.PROCEDURAL_CALL_PARAMS)) {
					changed = true;
				}
			}
		}

		// align parameters of "RAISE [RESUMABLE] EXCEPTION TYPE cx_class ... EXPORTING
		// ...", "RAISE SHORTDUMP TYPE cx_class ... EXPORTING ..."
		// and "RAISE EVENT evt [EXPORTING p1 = a1 p2 = a2 ...]";
		// also processes cases with USING MESSAGE or MESSAGE ..., but only aligns
		// starting from the EXPORTING keyword
		if (firstCode.matchesOnSiblings(true, "RAISE", TokenSearch.makeOptional("RESUMABLE"), "EXCEPTION", "TYPE",
				TokenSearch.ANY_IDENTIFIER, TokenSearch.ASTERISK, "EXPORTING")
				|| firstCode.matchesOnSiblings(true, "RAISE", "SHORTDUMP", "TYPE", TokenSearch.ANY_IDENTIFIER,
						TokenSearch.ASTERISK, "EXPORTING")
		...
		// align parameters of "IMPORT cluster" and "EXPORT cluster" (as well as some
		// obsolete forms that only differ behind the parameter list):
		...
		// skip cases with a keyword after IMPORT|EXPORT (e.g. IMPORT DIRECTORY INTO) as
		// well as dynamic cases like "IMPORT (ptab)..."
		if (firstCode.matchesOnSiblings(true, "IMPORT", TokenSearch.ANY_IDENTIFIER, "=|TO", TokenSearch.ASTERISK,
				"FROM")
				|| firstCode.matchesOnSiblings(true, "EXPORT", TokenSearch.ANY_IDENTIFIER, "=|FROM",
						TokenSearch.ASTERISK, "TO")) {
			...
			if (endToken != null) {
				int baseIndent = determineBaseIndentForRaise(command, parentToken);
				if (alignParams(code, command, parentToken, endToken, baseIndent, baseIndent,
						ContentType.IMPORT_OR_EXPORT)) {
					changed = true;
				}
			}
		}
		...
		if (firstCode.matchesOnSiblings(true, "READ|DELETE", "TABLE")) {
			...
			if (identifier != null) {
				...
				// for BINARY SEARCH, REFERENCE INTO, TRANSPORTING NO FIELDS, but because of
				// .getLastTokenOnSiblings, we keep it simple
				Token end = identifier.getLastTokenOnSiblings(true, TokenSearch.ASTERISK,
						"BINARY|INTO|ASSIGNING|REFERENCE|TRANSPORTING|.");
				if (end != null && identifier != end) {
					int baseIndent = (identifier.lineBreaks > 0) ? identifier.getStartIndexInLine()
							: identifier.getPrev().getEndIndexInLine() + 1;
					if (alignParams(code, command, identifier.getPrev(), end, baseIndent, baseIndent,
							ContentType.TABLE_KEY)) {
						changed = true;
					}
				}
			}
...
//				TokenSearch.ANY_IDENTIFIER, "IN")) {
//			Token parentToken = firstCode.getNextTokenOfTypeAndText(TokenType.KEYWORD, "FOR").getPrevCodeToken()
//					.getPrevCodeSibling();
//			int baseIndent = parentToken.getStartIndexInLine();
...
//		}
		...
		// cases!)
		// if line starts are allowed left of an assignment operator to preserve maximum
		// line length, perform 2 passes,
		// because we can't predict in advance what happens to inner constructs, and the
		// first pass may prove too radical
		// (e.g. if inner lists had to be broken into multiple lines even with the
		// minimum indent); this can then be revised
		// in the second pass that moves the construct back from the 'minimum indent' to
		// the 'base indent' or even behind the call
		...
		for (int pass = 0; pass < passCount; ++pass) {
			...
			while (token != null) {
				...
				if (end != null) {
					...
					// determine the base indent and the minimum indent (which may be left of an
					// assignment operator);
					// one of these indents is used in case putting parameters etc. behind the
					// opening bracket would exceed line length
					...
					if (contentLeftOfAssignOp == ContentLeftOfAssignOp.ALWAYS) {
						...
						// keep different values for base indent and minimum indent to decide depending
						// on content width
					}

					if (contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR
							&& allRowsAlignedForParent.contains(token.getParent())) {
			...
			// only do a second pass if something was changed - otherwise the second pass
	...
	private Token getFirstComponentOfTableKey(Token firstCode) {
		Token keyToken = firstCode.getLastTokenOnSiblings(true, TokenSearch.ASTERISK, "WITH",
				TokenSearch.makeOptional("TABLE"), "KEY");
		if (keyToken == null)
			return null;
	...
	private int determineBaseIndentForRaise(Command command, Token parentToken) {
		...
		// if before the EXPORTING section, there is any keyword at line start (esp.
		// MESSAGE), make baseIndent relative to that keyword
		// (esp. for cases of RAISE ... MESSAGE ... which may have been processed by the
		// AlignWithSecondWordRule) - example:
		...
		if (token != null && token.isKeyword() && token.lineBreaks > 0) {
			// determineTableStart will put EXPORTING at the position baseIndent +
			// ABAP.INDENT_STEP, therefore subtract it for now:
	...
	private int determineBaseIndent(Token token, ContentType contentType) {
		if (contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR || contentType == ContentType.GROUP_KEY) {
			// rows in a VALUE or NEW constructor expression were already moved when the
			// outer VALUE or NEW constructor itself was processed
			return token.getStartIndexInLine();
		} else {
			// parameters in a functional call or a constructor expression:
			// move to the beginning of the call chain, while that is on the same line; for
			// example,
			// move to the "lo_factory..." Token in "lv_result = lo_factory=>get(
			// )->get_utility( )->any_method( iv_param = ..."
			Token testToken = token;
			while (!testToken.isFirstTokenInLine() && testToken.closesLevel() && testToken.getPrev() != null
					&& testToken.getPrev().getOpensLevel()) {
				testToken = testToken.getPrev();
			}
			// move to the beginning of CALL METHOD
			while (!testToken.isFirstTokenInLine() && testToken.getPrev() != null
					&& testToken.getPrev().isAnyKeyword("CALL", "METHOD")) {
				testToken = testToken.getPrev();
			}
	...
	private int determineMinimumIndent(Token token, ContentType contentType) {
		if (contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR || contentType == ContentType.GROUP_KEY) {
			...
			// return the start index of the first token in same line; however, do not go to
			// parent level, e.g. in case of
			// "ls_struc = VALUE #( inner_struc = VALUE #( component = '1' ...", keep
			// "component" right of "inner_struc"
			Token testToken = token;
			while (testToken.lineBreaks == 0 && testToken.getPrev() != null && testToken.getPrev() != token.getParent())
				testToken = testToken.getPrev();

			// however, if the start of the line assigns to a very short variable (e.g. "exp
			// = ..." or "act = ..."),
			// move the minimum indent right of the assignment operator, since otherwise,
			// the layout looks awkward
			if (testToken.isIdentifier() && testToken.getTextLength() <= 4) {
				Token assignOp = testToken.getNext();
				if (assignOp != null && assignOp.isAssignmentOperator() && assignOp.lineBreaks == 0
						&& assignOp.spacesLeft <= 2) {
	...
	public final boolean alignParams(Code code, Command command, Token parentToken, Token endToken, int baseIndent,
			int minimumIndent, ContentType contentType) throws UnexpectedSyntaxAfterChanges {
		boolean changed = false;

		// -------------------------------------------------------------------
		// 1. prepare alignment

		// build an AlignTable and a list of 'otherLineStarts' (i.e. comment lines,
		// inner parentheses)
		...
		boolean alignAcrossTableRows = configAlignAcrossTableRows.getValue();
		try {
			// usually, we only consider the range from parentToken to endToken; however,
			// for table constructors, the components
			// of ALL rows ( ... ) ( ... ) can be added to the same AlignTable; in this
			// case, parentToken is only the start of the FIRST row
		...
		// if configured, join assignment operators and expressions into previous
		// columns, thus preventing the alignment
		// of assignments, or removing existing alignments
		if (!configAlignAssignments.getValue()) {
			Columns[] columnsToJoinIntoPrevious = new Columns[] { Columns.LET_ASSIGNMENT_OP, Columns.LET_EXPRESSION,
					Columns.ASSIGNMENT_OP, Columns.EXPRESSION };
			try {
		...
		TableStart tableStart = determineTableStart(parentToken, baseIndent, minimumIndent, contentType, table,
				otherLineStarts);

		// determine whether to keep the whole content of the parentheses / brackets on
		// one line

		boolean keepOnSingleLine = false;
		Token parentTokenOfFirstLine = null;
		if (contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR && alignAcrossTableRows) {
			// for rows that can be kept on one line, remove the corresponding assignments
			// from the AlignTable
			for (Token curParent : parentTokens) {
				if (determineKeepOnSingleLine(curParent, curParent.getNextSibling(), tableStart.earlyIndent, true)) {
					...
					// remember the parentToken of the first line that remains in the AlignTable (if
					// any)
					parentTokenOfFirstLine = curParent;
				}
			}
		} else {
			// only the range parentToken ... endToken is included in the AlignTable;
			// determine whether it can be kept on one line
			boolean forTabularStyle = (contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR
					|| isTableOfSingleComponents(parentToken, endToken));
		...
		// a) align the table - except when it contains multiple components in one line,
		// or when its components and values
		// may be manually aligned across multiple table rows (even with one component
		// only), as in
		// VALUE #( ( id = 1 )
		// ( id = 100 ) ).
		boolean skipAlign = keepOnSingleLine
				&& (table.getLineCount() > 1 || contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR);
		...
		if (!table.isEmpty() && !skipAlign) {
			...
			if (tableStart.forceTableToNextLine) {
				...
				startLineBreaks = 0;
			} else if (tableStart.continueOnSameLine && table.getFirstToken().lineBreaks == 0
					&& table.getFirstTokenColumnIndex() != Columns.PARAMETER.getValue()) {
				// e.g., keep COMPONENTS on the same line in "its_any[ KEY seckey COMPONENTS
				// comp1 = ..." and only break for "comp2 = ..."
				startLineBreaks = 0;
			} else {
				startLineBreaks = 1;
			}
		...
		boolean mayAlignAsteriskComments = !table.isEmpty() && !parameterColumn.isEmpty() && parameterIndent > 0
				&& !assignmentOpColumn.isEmpty() && assignmentOpIndent > 0 && !expressionColumn.isEmpty()
				&& expressionIndent > 0;

		// b) align other lines that were found inside the parentheses or brackets: "
		// comment lines, inner parentheses, assignments in * comment lines
		// If there is a LET ... IN expression, align with the parameters (which could
		// be right of the .startIndent of the "IN" keyword)
		...
		for (Token other : otherLineStarts) {
			if (other.textEquals("(")) { // stand-alone parenthesis, as opposed to "method_name(" or "type_name(" or
				...
				// align commented-out assignments, too (if 'effective indent' positions could
				// be retrieved from the AlignTable)
				...
				if (curIndent != otherLineIndent) {
					...
					if (other.isKeyword("WHERE") && addIndentEnd.textEquals("(")) {
						...
						// closing parenthesis found at line start; this can happen if
						// ClosingBracketsPositionRule is deactivated
						...
						if (next != null && next.lineBreaks == 0 && next.textEquals(")")) {
							// keep multiple closing parentheses where they are, because in this coding
							// style, their line start position
							// usually depends on the line start of the opening section that corresponds to
				...
				// if applicable, adjust the line breaks of the first 'otherLine', e.g. moving
				// it behind the parent token
...
//	}
	...
	private Token findStartOfNextRow(Token endOfRow) throws UnexpectedSyntaxException {
		...
		// arithmetic_expression' between rows
	...
	public static HashMap<String, Term> getFunctionalCallParams(Token parentToken) throws UnexpectedSyntaxException {
		...
		ArrayList<Token> otherLineStarts = new ArrayList<Token>();
		buildAlignTable(table, parentToken, parentToken.getNextSibling(), ContentType.FUNCTIONAL_CALL_PARAMS,
				otherLineStarts);
	...
	public static void buildAlignTable(AlignTable table, Token parentToken, Token end, ContentType contentType,
			ArrayList<Token> otherLineStarts) throws UnexpectedSyntaxException {
		table.getColumn(Columns.ASSIGNMENT_OP.getValue()).rightAlign = true; // if both = and ?= appear, align the "="
																				// and make the "?" stand out

		Token token = parentToken.getNext(); // parentToken is the opening parenthesis or bracket
		if (token == null)
			return;

		// consider the special case of a functional call with only an expression, but
		// no actual parameter specified, e.g.
		// any_method( VALUE #( ... ) ).
		...
		boolean isReceiveResults = (contentType == ContentType.PROCEDURAL_CALL_PARAMS
				&& parentToken.getParentCommand().firstCodeTokenIsKeyword("RECEIVE"));
		...
		while (token != null && token != end && token.getNext() != null) {
			if ((token.isIdentifier() || token.isKeyword("OTHERS"))
					&& (token.getPrevCodeSibling() == null || !token.getPrevCodeSibling().isKeyword("FOR")) // exclude
																											// conditional
																											// iteration
																											// "FOR var
																											// = rhs
																											...
																											// ..."
					&& token.getNext() != null
					&& (token.getNext().isAssignmentOperator() || contentType == ContentType.IMPORT_OR_EXPORT
							&& token.getNext().isAnyKeyword("TO", "FROM"))
					&& token.getNext().getNext() != null && !token.getNext().getNext().isComment()) {

				// identify the parts of the assignment "parameter = term" (and possibly the
				// keyword like "EXPORTING" preceding it)
				...
				if (keyword != null) {
					if (contentType == ContentType.GROUP_KEY || contentType == ContentType.IMPORT_OR_EXPORT) {
						...
						keyword = null;
					} else if (keyword.isAnyKeyword("EXPORTING", "IMPORTING", "TABLES", "CHANGING", "RECEIVING",
							"EXCEPTIONS")) {
				...
				// the assignment operator is usually "=", but may for "IMPORT ..." and "EXPORT
				// ..." may as well be the keyword "TO" or "FROM"
				Token assignmentOp = parameter.getNextCodeSibling(); // there may be a comment line between EXPORTING
				...
				// for "RECEIVE RESULTS FROM FUNCTION func" consider the MESSAGE addition after
				...
				if (isReceiveResults && isInExceptions
						&& parameter.textEqualsAny("system_failure", "communication_failure")) {
				...
				if (keyword != null) {
					line.setCell((isInLetExpression ? Columns.LET_KEYWORD : Columns.KEYWORD).getValue(),
							new AlignCellToken(keyword));
				}
				line.setCell((isInLetExpression ? Columns.LET_PARAMETER : Columns.PARAMETER).getValue(),
						new AlignCellToken(parameter));
				line.setCell((isInLetExpression ? Columns.LET_ASSIGNMENT_OP : Columns.ASSIGNMENT_OP).getValue(),
						new AlignCellToken(assignmentOp));
				line.setCell((isInLetExpression ? Columns.LET_EXPRESSION : Columns.EXPRESSION).getValue(),
						new AlignCellTerm(expression));
				...
				if (token.lineBreaks == 0) {
					...
					// "IN" at line start is put to the keyword column of the following content (NOT
					// the LET_KEYWORD column)
					AlignLine line = table.addLine();
			...
			} else if (contentType == ContentType.ROW_IN_VALUE_OR_NEW_CONSTRUCTOR
					&& (token.isIdentifier() || token.isLiteral())) {
				// in VALUE or NEW constructors, rows may not contain assignments, but only
				// - the expression, e.g. "lt_char = VALUE #( ( 'A' ) ( 'B' ) ( lv_char ) )"
				// (for tables without structured line type)
				// - a variable for a whole structure, e.g. "lt_table = VALUE #( ( ls_struc ) (
				// comp = 1 ) )"
				// Note that only the second case can be mixed with other lines that contain
				// assignments, and if alignment across
				// table rows is activated, 'LS_STRUC' should be aligned with the component
				// 'COMP', not with the value '1'; therefore,
				// the .PARAMETER column is used in all non-assignment cases, not the
				// .EXPRESSION column
				Term expression = Term.createArithmetic(token);
				AlignLine line = continueLastLine ? table.getLastLine() : table.addLine();
				...
				if (token == parentToken.getNext() || token.lineBreaks > 0) {
					if (token.isPseudoCommentAfterCode()) {
						// avoid moving pseudo comments to the next line if they refer to the line of
						// the parentToken
					} else if (token.isCommentAfterCode() && contentType != ContentType.CONSTRUCTOR_EXPR
...
//						table.addLine();
				...
				} else if (token.textEquals("(") && token.lineBreaks == 0 && token.getPrevCodeSibling() != null
						&& token.getPrevCodeSibling().isKeyword("WHERE")) {
					// skip this case of a logical expression in parentheses after WHERE, because it
					// is NOT a table row:
					// 'VALUE type( FOR ... IN ... WHERE ( log_exp ) ... ).'
				} else if (token.textEquals("(")) {
					// always store the start of rows in VALUE or NEW constructors (even if they do
					// not yet start a line)
					otherLineStarts.add(token);
					tableEndsWithOtherLine = true;
				}
				continueLastLine = false;
		...
		if (!tableEndsWithOtherLine) {
			// consider the space needed for closing brackets and anything attached to them
			// (except comments) by adding the
			// required width to the end of the last table line. Example: in "lts_table[
			// param = ... ]-component.", the length
			// of "]-component." must be considered for total line length
			int addWidth = 0;
			token = end;
			while (token != null && token.lineBreaks == 0 && !token.isComment()
					&& (token.closesLevel() || token.spacesLeft == 0)) {
	...
	private void setForcedLineBreaks(AlignTable table, ContentType contentType) {
		...
		if (hasLetExpression) {
			...
			// the LET_IN_KEYWORD column is only filled if "IN" is at the beginning of a
			// line;
			// however, even if it is NOT filled, it will forced the indent back to
			// "basicIndent + 0"
			table.getColumn(Columns.LET_IN_KEYWORD.getValue()).setForceIndent(0);

		} else {
			// if configured, setup the AlignTable to force a line break after EXPORTING,
			// IMPORTING etc. keywords
			// however, do not force a line break if the table continues behind a call
			int maxParamCount = 0;
			if (contentType == ContentType.PROCEDURAL_CALL_PARAMS)
				maxParamCount = (configMaxParamCountBehindProceduralCall.getValue() > 0 ? 1 : 0); // alternative:
																									// maxParamCount =
																									// config...getValue();
			else if (contentType == ContentType.FUNCTIONAL_CALL_PARAMS)
				maxParamCount = (configMaxParamCountBehindFunctionalCall.getValue() > 0 ? 1 : 0); // alternative:
																									// maxParamCount =
																									// config...getValue();
			...
			if (!keywordColumn.isEmpty() && forceLineBreakAfterKeywords(contentType)
					&& table.getLineCount() > maxParamCount) {
	...
	private boolean forceLineBreakAfterKeywords(ContentType contentType) {
		return (contentType == ContentType.PROCEDURAL_CALL_PARAMS
				&& configPutProceduralCallKeywordsOnOwnLine.getValue())
				|| (contentType == ContentType.FUNCTIONAL_CALL_PARAMS
						&& configPutFunctionalCallKeywordsOnOwnLine.getValue());
	}

	private TableStart determineTableStart(Token parentToken, int baseIndent, int minimumIndent,
		...
		// note that continueOnSameLine may refer to the table (if it starts directly
		// after parentToken) or to otherLineStarts
		...
		// exceptions: in some cases, indent at the current position of the first
		// assignment:
		// - table expression with KEY, e.g. "lts_defitem_src[ KEY latest_defitem
		...
		if (firstTokenInTable != null) {
			if ((!continueOnSameLine && firstTokenInTable.getPrev().isKeyword("WHERE"))
					|| firstTokenInTable.isKeyword("COMPONENTS"))
				startIndent = firstTokenInTable.getStartIndexInLine();
		}

		// determine the total width of the table, as well as the rows in a VALUE or NEW
		// constructor
		...
		for (Token token : otherLineStarts) {
			if (token.textEquals("(")) {
				try {
					...
					// if the row is on one line, only consider its length if it has a chance of
					// remaining on one line
					if (!term.isOnSingleLine()
							|| minimumIndent + rowWidth <= configMaxLineLengthForSingleLine.getValue())
		...
		// determine whether base indent can be used, or even minimum indent is required
		// (if they are different)
		int earlyIndent = ((baseIndent + addIndent + tableWidth <= configMaxLineLength.getValue()) ? baseIndent
				: minimumIndent) + addIndent;

		// move assignments to the left if line length is exceeded // TODO: add
		// configuration to prevent/allow/enforce this (possibly depending on main
		// keyword)
		...
		if (startIndent + tableWidth > configMaxLineLength.getValue()) {
			if (startInParentheses + tableWidth <= configMaxLineLength.getValue()) {
				// even if the table does not directly start after the parentToken, move the
				// table to the next line,
				// e.g. in the above example "lts_defitem_src[ KEY latest_defitem COMPONENTS
				// condition_type = ..."
				forceTableToNextLine = true;
				startIndent = startInParentheses;
			} else {
				// move the assignments to the earliest possible position (if this is an
				// improvement)
				if (earlyIndent < startIndent) {
					continueOnSameLine = false;
					startIndent = earlyIndent;
				}
				// if the line length is still exceeded, force a line break after EXPORTING,
				// IMPORTING etc., if applicable
		...
		// depending on configuration, move to the next line and the minimum indent once
		// a certain number of parameters is exceeded
		if (contentType == ContentType.PROCEDURAL_CALL_PARAMS) {
	...
	private boolean determineKeepOnSingleLine(Token parentToken, Token end, int earlyIndent, boolean forTabularStyle) {
		...
		ComponentsOnSingleLine keepOnSingleLine = forTabularStyle
				? ComponentsOnSingleLine.forValue(configKeepComponentsOnSingleLine.getValue())
				: ComponentsOnSingleLine.forValue(configKeepOtherOneLiners.getValue());
		switch (keepOnSingleLine) {
		case ALWAYS:
			return true;
		case IF_BELOW_MAX_LINE_LENGTH:
			int maxLineLength = forTabularStyle ? configMaxLineLengthForSingleLine.getValue()
					: configMaxLineLength.getValue();
	...
	private boolean isTableOfSingleComponents(Token parentToken, Token end) {
		// returns true for a table with single components like VALUE #( ( 1 ) ( 2 ) ( 3
		// ) ) or
		// VALUE #( ( lo_any ) ( lo_other ) ( lo_third ) ), which may be kept on one
		// line, although it is not a table row
	...
	private boolean moveRowInValueOrNewConstructor(Token openingToken, int baseIndent, boolean keepOnSingleLine)
		...
		if (openingToken.lineBreaks > 0) {
			if (openingToken.getPrev() == openingToken.getParent()
					&& openingToken.getParent().getEndIndexInLine() + 1 == baseIndent) {
		...
		// if the parenthesis spans multiple lines, add (or reduce) their indentation,
		// too
		if (changed) {
			int newStartIndex = openingToken.getStartIndexInLine();
			openingToken.getParentCommand().addIndent(newStartIndex - oldStartIndex, oldStartIndex,
					openingToken.getNext(), openingToken.getNextSibling());
		}
	...
	private boolean alignAsteriskComment(Token comment, int parameterIndent, int assignmentOpIndent) {
		...
		sb.append(bits[0]); // any number of asterisks, by which the remaining text will be additionally
